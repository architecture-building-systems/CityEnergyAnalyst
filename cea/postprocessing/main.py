...
dict_availabilities = {'centralized': 'Elec_renewables', 'centralized_ALL': 'All_renewables'}
    ...
    for scenario in scenarios:
        for availability in components_available:
            ...
            if availability == 'centralized_ALL':
                ...
                cluster_path = os.path.join(save_directory, context, scenario, "clustering_analysis_ALL")
            else:
            ...
            if 'Scenario' not in selected_systems.columns:
                ...
                if context == 'THESIS_TEST_CASES_RENEWABLES':
                    selected_systems.insert(2, 'Availability', dict_availabilities[availability])
                else:
                    selected_systems.insert(2, 'Availability', 'No_renewables')
            else:
                selected_systems['Scenario'] = scenario
                if context == 'THESIS_TEST_CASES_RENEWABLES':
                    selected_systems['Availability'] = dict_availabilities[availability]
                else:
                    selected_systems['Availability'] = 'No_renewables'

            # Combine the selected systems from different scenarios and contexts in one DataFrame
            if 'Scenario' not in percentage_change.columns:
                percentage_change.insert(1, 'Scenario', scenario)
                if context == 'THESIS_TEST_CASES_RENEWABLES':
                    percentage_change.insert(2, 'Availability', dict_availabilities[availability])
                else:
                    percentage_change.insert(2, 'Availability', 'No_renewables')
            else:
                percentage_change['Scenario'] = scenario
                if context == 'THESIS_TEST_CASES_RENEWABLES':
                    percentage_change['Availability'] = dict_availabilities[availability]
                else:
                    percentage_change['Availability'] = 'No_renewables'
            ...
            selected_systems_structure = process_energy_system_data(main_directory, selected_systems, filename_structure,
                                                                    ...
                                                                    availability, dict_availabilities)
...
for scenario in scenarios:
    ...
    # radar_chart(selected_systems_structure, plots_path_combined, scenario)
    stacked_bar_chart(selected_systems_structure, plots_path_combined, scenario)
