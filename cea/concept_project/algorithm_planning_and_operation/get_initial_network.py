...
def calc_substation_location():
    ...
    output_substations_shp = LOCATOR + SCENARIO + 'inputs/networks/nodes_buildings.shp'
...
def connect_building_to_grid():
    #import/ export paths
    input_substations_shp = LOCATOR + SCENARIO + 'inputs/networks/nodes_buildings.shp'
...
def get_hourly_power_demand_per_building(points_on_line):
    demand_folder = LOCATOR + SCENARIO + '/outputs/data/demand/'
    ...
    for idx_node, node in points_on_line.iterrows():
        if node['Building'] != None:
            ...
            hourly_power_demand_per_building[building_name] = pd.read_csv(demand_folder + building_name + '.csv')
...
def main():
    ...
    hourly_power_demand_per_building = get_hourly_power_demand_per_building(points_on_line_processed)
