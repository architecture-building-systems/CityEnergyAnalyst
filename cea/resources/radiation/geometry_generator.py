...
def _planar_quad_points(occface: TopoDS_Face) -> list[tuple[float, float, float]] | None:
    """The face's 4 corners, in order, if it is a simple planar quadrilateral -- ``None`` otherwise.

    A wall face from `calc_solid`'s loft is always such a quad (two footprint points x two z-levels), which is
    what `create_hollowed_facade` below relies on to avoid an OCC boolean cut. Any other shape falls back to it.
    """
    points = fetch.points_frm_occface(occface)
    if len(points) != 4:
        return None
    if abs(calculate.face_area(occface) - calculate.face_area(construct.make_polygon(points))) > 1E-6:
        return None  # non-planar or self-intersecting: the flat polygon through these points has a different area
    return points
...
def create_hollowed_facade(surface_facade: TopoDS_Face,
    ...
    """
    Removes the window from the facade to create a hollowed facade, as a list of faces covering the remaining area.

    For the common case -- a planar quadrilateral facade with a window that is a uniformly scaled copy of it (as
    `create_windows` produces) -- the hollowed facade is exactly 4 trapezoids, one per edge, running from each
    outer corner to the corresponding (nearest) inner corner. This is exact (no tolerance, no discarded slivers)
    and, unlike an OCC boolean difference followed by `BRepMesh_IncrementalMesh` triangulation, does not depend on
    the OCCT build: a symmetric hole in a rectangle is exactly the kind of shape where a triangulator's
    floating-point tie-breaking differs between platforms, which is what made wall sensor grids differ across
    OSes (see #4080). Other shapes (non-planar or non-quad facades) fall back to the OCC-based approach.
    ...
    :type window: OCCface
    :return: the hollowed facade as a mesh (list of `OCCface` triangles or trapezoids).
    :rtype: list[OCCface]
    :return: the hollowed facade created by the window subtraction.
    :rtype: OCCface
    """
    outer_pts = _planar_quad_points(surface_facade)
    window_pts = _planar_quad_points(window) if outer_pts is not None else None
    if outer_pts is not None and window_pts is not None:
        frame = [construct.make_polygon([outer_pts[i], outer_pts[(i + 1) % 4],
                                        min(window_pts, key=lambda p: calculate.distance_between_2_pts(p, outer_pts[(i + 1) % 4])),
                                        min(window_pts, key=lambda p: calculate.distance_between_2_pts(p, outer_pts[i]))])
                for i in range(4)]
        # Verify against the OCC-computed areas rather than trust the corner-matching blindly: falls back safely
        # if a real facade ever breaks an assumption above (e.g. a non-convex quad) instead of silently mis-areaing it.
        expected_area = calculate.face_area(surface_facade) - calculate.face_area(window)
        if abs(sum(calculate.face_area(f) for f in frame) - expected_area) < 1E-6 * max(expected_area, 1.0):
            return frame, fetch.topo2topotype(construct.boolean_difference(surface_facade, window))
