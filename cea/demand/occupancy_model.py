...
def calc_schedules(list_uses, archetype_schedules, bpr, archetype_values, stochastic_occupancy):
    ...
    :type archetype_values: DataFrame
    ...
    for use in list_uses:
        people_per_square_meter += bpr.occupancy[use] * archetype_values.loc[use, 'people']
    ...
    for num, use in enumerate(list_uses):
        if bpr.occupancy[use] > 0:
            ...
            if archetype_values.loc[use, 'people'] != 0:  # do not consider when the value is 0
                current_schedule = np.rint(np.array(archetype_schedules[use]['people']) * archetype_values.loc[
                    use, 'people'] * current_share_of_use * bpr.rc_model['NFA_m2'])
                ...
                for label in occupant_schedules:
                    ...
                    if current_archetype_values[use] != 0:  # do not consider when the value is 0
                        normalizing_values[label] += current_archetype_values[use] * archetype_values.loc[
                            use, 'people'] * current_share_of_use / people_per_square_meter
...
def calc_stochastic_schedules(archetype_schedules, archetype_values, bpr, list_uses, people_per_square_meter):
    ...
    for num, use in enumerate(list_uses):
        ...
        if current_share_of_use > 0:
            ...
            unoccupied_times = np.array([i == 0 for i in archetype_schedules[use]['people']])
            normalized_schedule = make_normalized_stochastic_schedule(current_stochastic_schedule,
                                                                      archetype_schedules[use]['people'],
                                                                      unoccupied_times)
        ...
        if archetype_values[use] != 0:  # do not consider when the value is 0
            if occupancy[use] > 0:
                ...
                if schedule_code == 'water':
                    ...
                    share_time_occupancy_density = \
                        archetype_values[use] * current_share_of_use * archetype_occupants[use]
                else:
                    share_time_occupancy_density = archetype_values[use] * current_share_of_use
...
def read_schedules(archetypes_schedules):
    """
    ...
    occ = [archetypes_schedules['Weekday_1'].values[:24],
           archetypes_schedules['Saturday_1'].values[:24],
           archetypes_schedules['Sunday_1'].values[:24]]
    el = [archetypes_schedules['Weekday_2'].values[:24],
          archetypes_schedules['Saturday_2'].values[:24],
          archetypes_schedules['Sunday_2'].values[:24]]
    dhw = [archetypes_schedules['Weekday_3'].values[:24],
           archetypes_schedules['Saturday_3'].values[:24],
           archetypes_schedules['Sunday_3'].values[:24]]
    month = archetypes_schedules['month'].values[:12]

    # if use in {"INDUSTRIAL", "HOSPITAL", "LAB"}:
    if 'Weekday_4' in archetypes_schedules.columns:
        pro = [archetypes_schedules['Weekday_4'].values[:24],
...
def schedule_maker(dates, locator, list_uses):
    ...
    :return archetype_values: dataframe containing the values for occupant density (in people/m2), internal loads and
        ventilation demand for each occupancy type used in the project
    :rtype archetype_values: DataFrame
    """
