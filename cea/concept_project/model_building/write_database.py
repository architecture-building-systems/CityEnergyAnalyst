...
def prepare_folder(
    ...
    shutil.copy(
        os.path.join(
            scenario_data_path, '..', 'external', 'building_model', 'data', 'building_zone_constraint_profiles.csv'
        ),
        os.path.join(scenario_data_path, scenario, 'concept', 'building-definition')
    )
...
def write_building_zones(
    ...
    building_zones_df.to_csv(
        path_or_buf=os.path.join(scenario_data_path, scenario, 'concept', 'building-definition', 'building_zones.csv'),
        index=False
    )
...
def write_building_surfaces_interior(
    ...
    building_surfaces_interior_df.to_csv(
        path_or_buf=os.path.join(scenario_data_path, scenario, 'concept', 'building-definition', 'building_surfaces_interior.csv'),
        index=False
    )
...
def write_building_surfaces_adiabatic(
    ...
    building_surfaces_adiabatic_df.to_csv(
        path_or_buf=os.path.join(scenario_data_path, scenario, 'concept', 'building-definition', 'building_surfaces_adiabatic.csv'),
        index=False
    )
...
def write_building_surfaces_exterior(
            ...
            if zone_occupancy_df.loc[building][occupancy] > 0:
                for index, row in building_geometry_all[building].iterrows():
                    if row['TYPE'] == 'walls':
                        ...
                        print('Error: In the ' + building + '_geometry.csv file, the "TYPE" of surface '
    ...
    building_surfaces_exterior_df.to_csv(
        path_or_buf=os.path.join(scenario_data_path, scenario, 'concept', 'building-definition', 'building_surfaces_exterior.csv'),
        index=False
    )
...
def write_building_blind_types(
    ...
    building_blind_types_df.to_csv(
        path_or_buf=os.path.join(scenario_data_path, scenario, 'concept', 'building-definition', 'building_blind_types.csv'),
        index=False
    )
...
def write_buildings(
    ...
    buildings_df.to_csv(
        path_or_buf=os.path.join(scenario_data_path, scenario, 'concept', 'building-definition', 'buildings.csv'),
        index=False
    )
...
def write_building_surface_types(
    ...
    building_surface_types_df.to_csv(
        path_or_buf=os.path.join(scenario_data_path, scenario, 'concept', 'building-definition', 'building_surface_types.csv'),
        index=False
    )
...
def write_building_window_types(
    ...
    building_window_type_df.to_csv(
        path_or_buf=os.path.join(scenario_data_path, scenario, 'concept', 'building-definition', 'building_window_types.csv'),
        index=False)
...
def write_building_internal_gain_types(
    ...
    building_internal_gain_types_df.to_csv(
        path_or_buf=os.path.join(scenario_data_path, scenario, 'concept', 'building-definition', 'building_internal_gain_types.csv'),
        index=False
    )
...
def write_building_internal_gain_timeseries(
    ...
    internal_gain_timeseries_df.to_csv(
        path_or_buf=(
            os.path.join(scenario_data_path, scenario, 'concept', 'building-definition', 'building_internal_gain_timeseries.csv')
        ),
        index=False
    )
...
def write_weather(
    ...
    for index, row in weather_timeseries_initial_df.iterrows():
        ...
        if int(row['year']) != year:
            print(
                'Warning ! The year is not the same throughout the prediction horizon, please adapt the '
                'get_electricity_price.py file accordingly.'
            )
            quit()
...
def write_building_scenarios(
    ...
    building_scenarios_df.to_csv(
        path_or_buf=os.path.join(scenario_data_path, scenario, 'concept', 'building-definition', 'building_scenarios.csv'),
        index=False
    )
...
def write_building_parameter_sets(
    ...
    for building in buildings_names:
        if mean_floor_height_m[building] != mean_floor_height_0:
            print(
                'Warning: the district buildings do not all have the same mean floor height. Please modify the '
                'pre_write_building_infiltration_types and the pre_write_building_heat_capacity_types functions in '
                'the write_database.py file so that each building has an infiltration and a heat capacity that are'
                ' adapted to its mean floor height.'
            )
            quit()
    ...
    building_parameters_sets_df.to_csv(
        path_or_buf=os.path.join(scenario_data_path, scenario, 'concept', 'building-definition', 'building_parameter_sets.csv'),
        index=False
    )
...
def write_building_linearization_types(
    ...
    building_linearization_types_df.to_csv(
        path_or_buf=os.path.join(scenario_data_path, scenario, 'concept', 'building-definition', 'building_linearization_types.csv'),
        index=False
    )
...
def write_building_zone_types(
    ...
    building_zone_types_df.to_csv(
        path_or_buf=os.path.join(scenario_data_path, scenario, 'concept', 'building-definition', 'building_zone_types.csv'),
        index=False
    )
