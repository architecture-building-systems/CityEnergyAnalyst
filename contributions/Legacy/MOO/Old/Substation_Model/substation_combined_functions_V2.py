...
def design_inhouse_exchangers(Q_heating_design, Q_dhw_design, T_return_min_global, T_supply_max_global, mdot_internal_dhw_design, mdot_internal_heating_design, Q_cool_max, T_cool_return_max_global, T_cool_supply_min_global, mdot_internal_cool_design, U_cool, hex_type, U, cp):
...
def dhw_hex_operation(Q_dhw, hex_type, A_hex_dhw_design, mdot_dhw_design, mdot_internal_dhw, U, cp, k_dhw_design, mdot_DH_to_dhw, T_supply_max_all_buildings):
    """
    ...
    """ 
   # START REPLACE WITH:
    T_s1 = max(T_supply_max_all_buildings, 70 + 273.0)
    #T_s1 = 70 + 273.0
...
def heating_hex_operation(Q_heating, T_r2_heating, T_s2_heating, hex_type, A_hex_heating_design, mdot_heating_design, mdot_internal_heating, U, cp, k_heating_design, mdot_DH_to_heating, T_supply_max_all_buildings):
    """
...
def import_thermal_data(fName, DAYS_IN_YEAR, HOURS_IN_DAY):
    ...
    Q_spaceheat_dataframe = extract_csv(fName, "Qhs", DAYS_IN_YEAR)
    Q_cool_dataframe = extract_csv(fName, "Qcs", DAYS_IN_YEAR)
    ...
    T_cool_supply_dataframe = extract_csv(fName, "tscs", DAYS_IN_YEAR)
    T_cool_return_dataframe = extract_csv(fName, "trcs", DAYS_IN_YEAR)
    
    Q_dhw_dataframe = extract_csv(fName, "Qwwf", DAYS_IN_YEAR)
    mdotcp_internal_dhw_dataframe = extract_csv(fName, "mcpww", DAYS_IN_YEAR)
    ...
    return Q_spaceheat_array, Q_heating_array, Q_dhw_array, Q_cool_array, Electr_array, T_supply_array, T_return_array, T_amb_array, T_return_min_global, T_supply_max_global, T_amb_max, Q_cool_max, Q_heating_design, Q_dhw_design, Q_spaceheat_design, mdot_internal_dhw_array, mdot_internal_dhw_design, mdot_internal_heating_array, mdot_internal_heating_design, mdot_internal_cool_array, mdot_internal_cool_design, T_cool_internal_supply_array, T_cool_internal_return_array, T_cool_internal_supply_min_global, T_cool_internal_return_max_global

    
    
        
def translate_J_plus_to_DH_requirements(DAYS_IN_YEAR, HOURS_IN_DAY, thermal_data, hex_design_data, hex_type, temperature_data, U, U_cool, cp, mdot_step_counter, mdot_cool_step_counter, T_cool_return_max_global, T_cool_supply_min_global):
        ...
        for k in range(DAYS_IN_YEAR):
            for j in range(HOURS_IN_DAY):
                for mdot_stepper in range(len(mdot_step_counter)): # loop for massflow iteration
                    ...
                    T_r1_dhw, mdot_dhw, A_hex_dhw_check = dhw_hex_operation(Q_dhw, hex_type, A_hex_dhw_design, mdot_dhw_design, mdot_internal_dhw, U, cp, k_dhw_design, mdot_step, T_supply_max_all_buildings_of_t)
                    ...
                    T_r1_heating, mdot_heating_req, T_s1_DH, A_hex_heating_check = heating_hex_operation(Q_heating, T_r2_heating, T_s2_heating, hex_type, A_hex_heating_design, mdot_heating_design, mdot_internal_heating, U, cp, k_heating_design, mdot_DH_to_heating, T_supply_max_all_buildings_of_t)
