        ...
        *,
        ...
        for year in years_sorted:
            ...
            if regenerate_building_properties:
                # Properties already regenerated by delete_unexisting_buildings_from_event_scenario
                # This block kept for backward compatibility with regenerate_building_properties=False
                pass
    ...
    def bake_states_from_log(self) -> None:
        ...
        for year in years:
            ...
            if year_recipe:
                ...
                print("  Applied cumulative modifications from timeline log")
            else:
...
def modify_state_construction(
    ...
    *,
    ...
    Returns:
        None
    """
    # Apply archetype modifications (includes property regeneration)
    ...
    main_locator = InputLocator(config.scenario)
    if log_data is None:
        log_data_for_reconciliation = load_log_yaml(
            main_locator, allow_missing=True, allow_empty=True, timeline_name=timeline_name
        )
        log_modifications(config, timeline_name, year_of_state, modify_recipe)
        # Reload to get the just-written modifications
        log_data_for_reconciliation = load_log_yaml(
            main_locator, allow_missing=True, allow_empty=True, timeline_name=timeline_name
        )
    else:
        _log_modifications_in_memory(log_data, year_of_state, modify_recipe)
        log_data_for_reconciliation = log_data
    
    # Update future state scenarios to include cumulative modifications
    _update_future_state_scenarios_after_year_event(
        config,
        timeline_name,
        year_of_state,
        log_data=log_data_for_reconciliation,
    )
    return None
...
def _apply_state_construction_changes(
    ...
    *,
    ...
    1. Modifies archetype assemblies in the databases
    2. Regenerates per-building property files (supply.csv, architecture.dbf, etc.) from the modified archetypes
...
def _update_future_state_scenarios_after_year_event(
    ...
    *,
    ...
    for future_year in affected_years:
        ...
        if missing_recipe:
            # Apply archetype modifications to future state
            modified = _apply_state_construction_changes(
                config,
                timeline_name,
                future_year,
