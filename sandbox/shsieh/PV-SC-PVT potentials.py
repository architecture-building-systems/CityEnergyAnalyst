...
#TODO: replace with new library that can convert shapefile to dataframe directly
...
#TODO: include SC panel and PVT panel
misc_losses = 0.10 # losses for a good system due to cabling, regulator and inverters
#TODO: check reference, used in calc_PVgeneration
...
#TODO: move to globalvar
angle_north = 122.5
#TODO: what is angle_north and worst_Sh
diffuseProp = T_G_day['diff'].mean()
transmittivity = T_G_day['ttr'].mean()
worst_sh = T_G_hour.loc[worst_hour,'Sh']  # Solar height at worst hour in [m]
...
#TODO: import hourly solar radiation
...
module_length = 1 #m # 1 for PV and 1 for solar collectors
EDM.optimal_angle_and_tilt(observers_data, latitude, worst_sh, worst_Az, transmittivity, diffuseProp, grid_side,
                           module_length, angle_north, Min_Isol, Max_Isol)
...
#calculate number of optima groups as number of optimal combinations.
...
# remove points with radiation less than 80% the maximum in the obesrvers_all #remove points towards north #calculate tilted area also
#create categories of tilted areas. categorize also potentials.
module_length = 2 #m # 1 for PV and 1 for solar collectors
EDM.optimal_angle_and_tilt(observers_data, latitude, worst_sh, worst_Az, transmittivity, diffuseProp, grid_side, module_length, angle_north, Min_Isol, Max_Isol)
#IMPORT OBSERVERS WITH FILTER AS DF
outpath = "observers.dbf"
arcpy.TableToTable_conversion(observers_data,locationtemp1,outpath)
observers_fin = dbf2df(locationtemp1+'\\'+outpath)
...
# remove points with radiation less than 80% the maximum in the obesrvers_all #remove points towards north #calculate tilted area also
#create categories of tilted areas. categorize also potentials.
module_length = 2 #m # 1 for PV and 2 for solar collectors
EDM.optimal_angle_and_tilt(observers_data, latitude, worst_sh, worst_Az, transmittivity, diffuseProp, grid_side, module_length, angle_north, Min_Isol, Max_Isol)
#IMPORT OBSERVERS WITH FILTER AS DF
outpath = "observers.dbf"
arcpy.TableToTable_conversion(observers_data,locationtemp1,outpath)
