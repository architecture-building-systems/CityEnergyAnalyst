...
def dhw_hex_operation(Q_dhw, hex_type, A_hex_dhw_design, mdot_dhw_design, mdot_internal_dhw, U, cp, k_dhw_design):
    ...
    #A_iteration = A_hex_dhw_design * 1.1

    #iteration_counter = 0
    
    
    
    #T_r1_calc = T_r1_min - 1
    
    #while T_r1_calc < T_r1_min and iteration_counter<10:
      #  iteration_counter += 1
    #T_r1 = T_r1_calc
    
    c1 = k_dhw_design  * A_hex_dhw_design * (T_s1 - T_s2) / (T_s2 - T_r2)**q 
    c2 = (T_s1 - T_s2) / (T_s2 - T_r2) 


    def fh(x):
        Eq = c1 * (x -1) / ( 1+ (1 - c2 *( x-1) + 0j)**q * scipy.log(x)) - Q**(1-q)
        return Eq

    alpha = scipy.optimize.brentq(fh, 0.1, 0.9)
    #print alpha, "alpha"
    ...
    #print "A_hex_dhw_design = ", A_hex_dhw_design
    #print ".",
...
def heating_hex_operation(Q_heating, T_r2_heating, T_s2_heating, hex_type, A_hex_heating_design, mdot_heating_design, mdot_internal_heating, U, cp, k_heating_design):
    """
    gives return temperature and massflow of building to the interface - heat-exchanger
    ...
    c1 = k_heating_design  * A_hex_heating_design * (T_s1 - T_s2) / (T_s2 - T_r2)**q 
    c2 = (T_s1 - T_s2) / (T_s2 - T_r2) 
    ...
    print A_iteration, "new Area", A_hex_heating_design,"design Area"
    ...
    while loopcheck_dhw < 1 and loopcheck_heating < 1:
        for k in range(DAYS_IN_YEAR):
            for j in range(HOURS_IN_DAY):
                ...
                T_r1_heating, mdot_heating, T_s1_DH, A_hex_heating_check = heating_hex_operation(Q_heating, T_r2_heating, T_s2_heating, hex_type, A_hex_heating_design, mdot_heating_design, mdot_internal_heating, U, cp, k_heating_design)
        ...
        if A_hex_heating_check > A_hex_heating_design: # check if the area is sufficient, if not - restart the loop
            ...
            print " -----------------------------"
            loopcheck_heating = 0
            k_heating_design = U * (mdot_heating**(-0.8) + mdot_internal_heating*(-0.8))
...
for i in range(len(building_list_array)):
    ...
    fName_result = "1039_" + building_list_array[i,0] + "_result.csv"
