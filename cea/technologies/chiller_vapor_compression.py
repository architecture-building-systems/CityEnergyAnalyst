...
from cea.constants import MIN_TEMP_DIFF_FOR_HEAT_PUMP_OPERATION_K, KELVIN_CONVERSION
...
def calc_VCC_COP(weather_data, load_types, centralized=True):
    ...
    T_cond_K = np.mean(weather_data['wetbulb_C']) + CHILLER_DELTA_T_APPROACH + CHILLER_DELTA_T_HEX_CT + KELVIN_CONVERSION
    ...
    if temp_diff < MIN_TEMP_DIFF_FOR_HEAT_PUMP_OPERATION_K:
        raise ValueError(
            ...
            f"Condenser temperature: {T_cond_K:.2f} K ({T_cond_K - KELVIN_CONVERSION:.2f} °C)\n"
            f"Evaporator temperature: {T_evap_K:.2f} K ({T_evap_K - KELVIN_CONVERSION:.2f} °C)\n"
            f"Temperature difference: {temp_diff:.2f} K\n\n"
            f"For valid chiller operation:\n"
            f"- T_cond must be > T_evap by at least {MIN_TEMP_DIFF_FOR_HEAT_PUMP_OPERATION_K} K\n"
