...
def Storage_Design(CSV_NAME, SOLCOL_TYPE, T_storage_old_K, Q_in_storage_old_W, locator,
    ...
    T_DH_return_array_K = Network_Data['T_sst_heat_return_netw_total_K'].values
    T_DH_supply_array_K = Network_Data['T_sst_heat_supply_netw_total_K'].values
    ...
    Solar_E_aux_SC_kWh = Solar_Data_SC[:,1]
    ...
    PV_PVT_kWh = Solar_Data_PVT[:,5]
    Solar_E_aux_PV_kWh = Solar_Data_PV[:,1]
    PV_PV_kWh = Solar_Data_PV[:,5]
    ...
    for solartype in range(3):
        ...
        if MS_Var.SOLCOL_TYPE_SC != "NONE" and fName == MS_Var.SOLCOL_TYPE_SC:
            Solar_Area_SC_m2, Solar_E_aux_SC_kWh, Solar_Q_th_SC_kWh, Solar_Tscs_th_SC, Solar_mcp_SC_kWperC, PV_SC_kWh, Solar_Tscr_th_SC_K\
        ...
        if MS_Var.SOLCOL_TYPE_PVT != "NONE" and fName == MS_Var.SOLCOL_TYPE_PVT:
            Solar_Area_PVT_m2, Solar_E_aux_PVT_kWh, Solar_Q_th_PVT_kWh, Solar_Tscs_th_PVT, Solar_mcp_PVT_kWperC, PV_PVT_kWh, Solar_Tscr_th_PVT_K \
        ...
        if MS_Var.SOLCOL_TYPE_PV != "NONE" and fName == MS_Var.SOLCOL_TYPE_PV:
            Solar_Area_PV_m2, Solar_E_aux_PV_kWh, Solar_Q_th_PV_kWh, Solar_Tscs_th_PV, Solar_mcp_PV_kWperC, PV_PV_kWh, Solar_Tscr_th_PV_K\
   ...
   # Solar_Area = Solar_Data_SC[0] * MS_Var.SOLAR_PART_SC + Solar_Data_PVT[0] * MS_Var.SOLAR_PART_PVT + Solar_Data_PV[0] * MS_Var.SOLAR_PART_PV
    Solar_E_aux_W = np.ravel(Solar_E_aux_SC_kWh * 1000 * MS_Var.SOLAR_PART_SC) + np.ravel(Solar_E_aux_PVT_kWh * 1000 * MS_Var.SOLAR_PART_PVT) \
    ...
    Q_SC_Wh = Solar_Q_th_SC_kWh * 1000 * MS_Var.SOLAR_PART_SC
    #print Q_SC, "Q_SC"
    Q_PVT_Wh = Solar_Q_th_PVT_kWh * 1000 * MS_Var.SOLAR_PART_PVT
    Q_SCandPVT_Wh = np.zeros(HOURS_IN_DAY*DAYS_IN_YEAR)
    
    #Q_SCandPVT = float(np.sum(Q_SC) + np.sum(Q_PVT))
    #Q_SCandPVT = Q_SC + Q_PVT
    for hour in range(len(Q_SCandPVT_Wh)):
        Q_SCandPVT_Wh[hour] = Q_SC_Wh[hour] + Q_PVT_Wh[hour]
        
    #print Q_SCandPVT
    #print "shape", np.shape(Q_SCandPVT)
    
    E_PV_Wh = PV_PV_kWh * 1000 * MS_Var.SOLAR_PART_PV
    E_PVT_Wh = PV_PVT_kWh * 1000  * MS_Var.SOLAR_PART_PVT
    ...
    while HOUR < HOURS_IN_DAY*DAYS_IN_YEAR:
        ...
        if T_storage_new >= MS_Var.T_ST_MAX-0.001: # no more charging possible - reject energy
            Q_in_storage_new = min(Q_in_storage_old_W, Storage_Data[0])
            Q_to_storage_final = max(Q_in_storage_new - Q_in_storage_old_W, 0)
            Q_rejected_fin[HOUR] = Q_uncontrollable_Wh - Storage_Data[3]
            T_storage_new = min(T_storage_old_K, T_storage_new)
            E_aux_ch = 0
            print "Storage Full!"
        ...
        Q_in_storage_old_W = Q_in_storage_new
        
        T_storage_fin[HOUR] = T_storage_new
        T_storage_old_K = T_storage_new
        ...
        Q_uncontrollable_fin[HOUR] = Q_uncontrollable_Wh
        
        
        E_aux_HP_uncontrollable_fin[HOUR] = float(E_aux_HP_uncontrollable_Wh)
        #print type(E_aux_HP_uncontrollable_fin[HOUR]),
        mdot_DH_fin[HOUR] = mdot_DH_afterSto
        
        Q_from_storage_fin[HOUR] = Q_DH_networkload_W[HOUR] - Q_missing
        
        if T_storage_new <= T_storage_min_K:
            T_storage_min_K = T_storage_new
    ...
    for hour in range(DAYS_IN_YEAR, HOURS_IN_DAY):
        ...
        E_consumed_total_without_buildingdemand[hour] = E_aux_ch[hour] + E_aux_dech[hour] + E_aux_HP_uncontrollable_Wh[hour]
    ...
    if STORE_DATA == "yes":
        ...
        results = pd.DataFrame(
            {"Q_storage_content_Wh":Q_storage_content_fin, 
             "Q_DH_networkload":Q_DH_networkload_W,
             ...
             "E_PV_Wh":E_PV_Wh,
             "E_PVT_Wh":E_PVT_Wh,
             "Storage_Size":STORAGE_SIZE_m3,
             "Q_SCandPVT_coldstream":Q_SCandPVT_Wh,
             "E_produced_total":E_produced_total,
             "E_consumed_total_without_buildingdemand":E_consumed_total_without_buildingdemand,
             "HPServerHeatDesignArray":HPServerHeatDesignArray_kWh,
             "HPpvt_designArray":HPpvt_designArray_Wh,
             "HPCompAirDesignArray":HPCompAirDesignArray_kWh,
             "HPScDesignArray":HPScDesignArray_Wh,
             "Q_rejected_fin":Q_rejected_fin,
             "P_HPCharge_max":P_HP_max_W
            })
