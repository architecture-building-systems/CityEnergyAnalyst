...
def heating_hex_design(Q_heating_max, T_return_min_global, T_supply_max_global, hex_type, U , cp) :
    ...
    T_s1 = np.maximum(60 + 273 + 5, T_supply_max_global + 5)    
    
    if hex_type == "LT":
        T_s1 = np.maximum(50 + 273 + 5, T_supply_max_global + 5)
...
def dhw_hex_design(Q_dhw_max, T_amb_max, hex_type, U, cp):
    ...
    if hex_type == "LT":
        T_s1 = np.maximum(45 + 273 + 5, T_supply_max_global + 5)
...
def design_inhouse_exchangers(Q_heating_design, Q_dhw_design, T_amb_max, T_return_min_global, T_supply_max_global, hex_type, U, cp):
    ...
    A_hex_heating_design, mdot_heating_design = heating_hex_design(Q_heating_design, T_return_min_global, T_supply_max_global, hex_type, U , cp)
    A_hex_dhw_design, mdot_dhw_design = dhw_hex_design(Q_dhw_design, T_amb_max, hex_type, U, cp) 
    
    return A_hex_heating_design, mdot_heating_design, A_hex_dhw_design, mdot_dhw_design

    
def dhw_hex_operation(Q_dhw, hex_type, A_hex_dhw_design, T_amb, mdot_dhw_design, U, cp):
    """
...
def import_thermal_data(fName, DAYS_IN_YEAR, HOURS_IN_DAY):
    ...
    T_supply_max_global = T_supply_array[(find_index_of_max(Q_heating_array))] 
    T_return_min_global = T_return_array[(find_index_of_max(Q_heating_array))] 
    
    # set design conditions for thermal loads 
    Q_heating_design = Q_heating_max
    Q_dhw_design = Q_dhw_max
    
    return Q_finalheat_array, Q_heating_array, Q_dhw_array, Q_cool_array, Electr_array, T_supply_array, T_return_array, T_amb_array, T_return_min_global, T_supply_max_global, T_amb_max, Q_cool_max, Q_heating_design, Q_dhw_design
...
def translate_J_plus_to_DH_requirements(DAYS_IN_YEAR, HOURS_IN_DAY, thermal_data, hex_design_data, hex_type, temperature_data, U, cp):
    ...
    for k in range(DAYS_IN_YEAR):
        for j in range(HOURS_IN_DAY):
            ...
            T_r1_dhw, mdot_dhw = dhw_hex_operation(Q_dhw, hex_type, A_hex_dhw_design, T_amb, mdot_dhw_design, U, cp)
