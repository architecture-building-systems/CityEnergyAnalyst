...
def calc_Qcdataf(locator, bpr: BuildingPropertiesRow, tsd: TimeSeriesData) -> TimeSeriesData:
    ...
    if scale_technology == "BUILDING":
        if energy_source == "GRID":
            t_source = (tsd.weather.T_ext + KELVIN_CONVERSION)

            # heat pump energy
            tsd.electrical_loads.E_cdata = np.vectorize(heatpumps.HP_air_air)(tsd.cooling_system_mass_flows.mcpcdata_sys, (tsd.cooling_system_temperatures.Tcdata_sys_sup + KELVIN_CONVERSION),
                                                                (tsd.cooling_system_temperatures.Tcdata_sys_re + KELVIN_CONVERSION), t_source)
