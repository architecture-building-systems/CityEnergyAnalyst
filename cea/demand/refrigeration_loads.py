...
def calc_Qref(locator, bpr: BuildingPropertiesRow, tsd: TimeSeriesData) -> TimeSeriesData:
    ...
    if scale_technology == "BUILDING":
        if energy_source == "GRID":
            ...
            # heat pump energy
            tsd.electrical_loads.E_cre = np.vectorize(heatpumps.HP_air_air)(tsd.cooling_system_mass_flows.mcpcre_sys, (tsd.cooling_system_temperatures.Tcre_sys_sup + KELVIN_CONVERSION),
                                                                (tsd.cooling_system_temperatures.Tcre_sys_re + KELVIN_CONVERSION), t_source)
