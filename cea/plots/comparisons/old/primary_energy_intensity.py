...
def primary_energy_intensity(data_frame, analysis_fields, title, output_path):
    ...
    layout = go.Layout(images=LOGO, title=title, barmode='stack',
                       yaxis=dict(title='Primary energy use intensity (non-renewable) [MJ Oil-eq/m2.yr]', domain=[0.35, 1]),
...
def calc_table(analysis_fields, data_frame):
    ...
    for field in analysis_fields:
        values_header.append("delta " + NAMING[field] + " [MJ Oil-eq/m2.yr]")

    values_header.append("delta total [MJ Oil-eq/m2.yr]")
    #create values of table
