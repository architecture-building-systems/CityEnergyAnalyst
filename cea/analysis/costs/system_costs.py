...
def costs_main(locator, config):
    ...
    for service in heating_services:
        try:
            fields_to_plot.extend(
                [service + '_total_capex', service + '_capex_yr', service + '_OM_yr', service + '_OM_m2yr'])
    ...
    for service in electricity_services:
        fields_to_plot.extend(
            [service + '_sys_total_capex', service + '_sys_capex_yr', service + '_sys_OM_yr', service + '_sys_OM_m2yr'])
    ...
    ### assign DHW and electricity services (appliances, lighting, etc.) to disconnected, assuming WW is always decentralized
    result['Opex_a_sys_disconnected_USD'] = result['Qww_sys_OM_yr'] + \
                                            result['GRID_sys_OM_yr'] + \
                                            result['PV_sys_OM_yr'] + \
                                            result['OIL_ww_sys_opex_yr'] + \
                                            result['NG_ww_sys_opex_yr'] + \
    ...
    if scale_technology.str.contains('BUILDING').sum():
        ...
        result['Opex_a_sys_disconnected_USD'] += result['QH_sys_OM_yr'] + \
                                                 result['OIL_hs_sys_opex_yr'] + \
                                                 result['NG_hs_sys_opex_yr'] + \
                                                 result['WOOD_hs_sys_opex_yr'] + \
                                                 result['COAL_hs_sys_opex_yr'] + \
                                                 result['GRID_hs_sys_opex_yr']
        ...
        result['Opex_a_sys_connected_USD'] += result['QH_sys_OM_yr'] + \
                                              result['OIL_hs_sys_opex_yr'] + \
                                              result['NG_hs_sys_opex_yr'] + \
                                              result['WOOD_hs_sys_opex_yr'] + \
                                              result['COAL_hs_sys_opex_yr'] + \
    ...
    if scale_technology.str.contains("BUILDING").sum():
        ...
        result['Opex_a_sys_disconnected_USD'] += result['QC_sys_OM_yr'] + \
                                                 result['GRID_cs_sys_opex_yr'] + \
                                                 result['GRID_cdata_sys_opex_yr'] + \
                                                 result['GRID_cre_sys_opex_yr']
        ...
        result['Opex_a_sys_connected_USD'] += result['QC_sys_OM_yr'] + \
                                              result['DC_cs_sys_opex_yr'] + \
                                              result['GRID_cs_sys_opex_yr'] + \
                                              result['GRID_cdata_sys_opex_yr'] + \
                                              result['GRID_cre_sys_opex_yr']
    ...
    result = result[['Name'] + fields_to_plot]
    # result = result.loc[:, (result != 0).any(axis=0)] #delete all columns which are 0
