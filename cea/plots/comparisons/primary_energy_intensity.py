...
def primary_energy_intensity(data_frame, analysis_fields, title, output_path):
    ...
    layout = go.Layout(images=LOGO, title=title, barmode='stack',
                       yaxis=dict(title='Consumption of Fossil Fuels per Gross Floor Area [MJ Oil-eq/m2.yr]', domain=[0.35, 1]),
...
def calc_graph(analysis_fields, data_frame):
    ...
    for field in analysis_fields:
        ...
        trace = go.Bar(x=data_frame.index, y=y, name=field, text=total_perc_txt,
                       marker=dict(color=COLOR[field.split('_MJm2', 1)[0]]))
        graph.append(trace)
...
def calc_table(analysis_fields, data_frame):
    ...
    for field in analysis_fields+['total']:
        values_header.append("delta " + field + "[MJ Oil-eq/m2.yr]")
