...
def create_hollowed_facade(surface_facade, window):
    ...
    :type surface_facade: OCCtopology
    :param window: the window surface to be subtracted from the facade, generated by `create_windows`.
    :type window: OCCtopology
    :return: a tuple containing:

        1. the hollowed facade as a mesh (list of `OCCface` triangles).
        2. the hole face created by the window subtraction.
    :rtype: tuple[list[OCCface], OCCtopology]
    """
...
def calc_building_solids(buildings_df, geometry_simplification, elevation_map, num_processes):
    ...
    :param buildings_df: either the zone or surroundings buildings dataframe. 
        It should be read from either `"\\scenario\\inputs\\building-geometry\\zone.shp"` or `"\\scenario\\inputs\\building-geometry\\surroundings.shp"`, 
    ...
    :param geometry_simplification: the tolerance of simplification. 
        "All parts of a simplified geometry will be no more than tolerance distance from the original." (from `geopandas.GeoSeries.simplify`)
    :type geometry_simplification: float
    :param elevation_map: an instance of `ElevationMap` that contains the terrain elevation data, read from a raster file `"\\scenario\\inputs\\topography\\terrain.tif"`.
    :type elevation_map: ElevationMap
    :param num_processes: number of processes to use for parallel computation.
    :type num_processes: int
    :return: a tuple containing:

        1. a list of solids representing the building external shells, each made from footprint + vertical external walls + roof.
        2. a list of elevations corresponding to each building solid, where each elevation is a float value.
    :rtype: tuple[list[OCCsolid], list[float]]
    """
...
def process_geometries(geometry, elevation_map, range_floors, floor_to_floor_height):
    ...
    :return: a tuple containing:
        ...
        1. a solid representing the building, made from footprint + vertical external walls + roof.
        2. the elevation of the terrain at the footprint of the building.
    :rtype: tuple[OCCsolid, float]
    """
...
def burn_buildings(geometry, elevation_map, tolerance):
    """find the elevation of building footprint polygon by intersecting the center point of the polygon with the terrain elevation map, then move the polygon to that elevation.
    ...
    :return: a tuple containing:
        ...
        1. the bottom surface of the building, which is an `OCCface` object.
        2. the elevation of the terrain at the footprint of the building, which is a float value.
    :rtype: tuple[OCCface, float]
    """
    if geometry.has_z:
...
class ElevationMap(object):
    ...
    def generate_tin(self, tolerance=1e-6):
        ...
        :param tolerance: The minimal surface area of each triangulated face. Any faces smaller than the tolerance will be deleted. Defaults to 1e-6
        :type tolerance: float, optional
        :return: a list of OCCface triangles representing the TIN mesh.
        :rtype: list[OCCface]
        """
