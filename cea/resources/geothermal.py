...
from cea.constants import HOURS_IN_YEAR, SOIL_Cp_JkgK, SOIL_lambda_WmK, SOIL_rho_kgm3, PERMEABIL, PIEZ, DIST_PIEZ, HEAT_CAPACITY_OF_WATER_JPERKGK
...
def calc_geothermal_potential(locator, config):
    ...
    water_temperature_raise = config.shallow_geothermal.maximum_temperature_increase
    ...
    T_ground_K = calc_ground_temperature(T_ground_water, T_ambient_C, depth_m)
...
def calc_ground_temperature(T_ground_water, T_ambient_C, depth_m):
    """
    ...
    T_amplitude = abs((max(T_ambient_C)[0] + 273.15 - T_ground_water))  # K
    T_ground_K = calc_temperature_underground(T_amplitude, T_ground_water, conductivity_soil, density_soil, depth_m,
                                              heat_capacity_soil)
