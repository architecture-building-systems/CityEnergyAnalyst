...
### Key Parameters
- **Include zone**: Export zone building geometries. Keep this on — it also carries the
  typology information and the shapefile the others are geo-referenced against.
- **Include site**: Export the polygon drawn in the Create Scenario wizard, if any
...
- **Network name**: Choose an existing network layout to export. **Leave it empty to skip
  networks entirely.** Both the district heating and district cooling networks are exported if
  they exist for the layout you pick — there is no separate DH/DC switch.
...
3. Configure which elements to export:
   ...
   - Pick a network layout if you want the thermal networks exported, or leave it empty
4. Click **Run**
...
A scenario arriving from Grasshopper has geometry but none of the supporting data. Rather than
make you rebuild it, the import can copy that across from a scenario you already have.

- **Reference scenario name**: the scenario you originally exported with *Export to
  Rhino/Grasshopper*. The three options below copy from it. The current scenario is not offered.
- **Copy database**: bring the reference scenario's database across. This also runs the
  **Archetypes Mapper**, so the imported buildings come back with their envelope, HVAC, comfort,
  loads and supply populated.
- **Copy terrain**: bring the terrain file across. Use this only when every building in the new
  scenario — surroundings included — falls inside the reference scenario's terrain.
- **Copy weather**: bring the weather file across.
