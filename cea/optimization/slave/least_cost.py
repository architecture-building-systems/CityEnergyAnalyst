 ...
 #   """ Fixed order COST ALGORITHM STARTS """  # Run the Centralized Plant Operation Scheme
        ...
        while Q_therm_req_W > 1E-1:  # cover demand as long as the supply is lower than demand!
            if current_source == 'HP':  # use heat pumps available!
                ...
                if (MS_Var.HP_Lake_on) == 1 and Q_therm_req_W > 0 and gv.HPLake_allowed == 1:  # run Heat Pump Lake
                    ...
                    E_HPLake_req_W = Wdot_HPLake
                    E_coldsource_HPLake_W = Q_HPL_cold_primary
            ...
            if current_source == 'BoilerBase' and Q_therm_req_W > 0:
                ...
                if (MS_Var.Boiler_on) == 1:
                    ...
                    if Q_therm_req_W >= gv.Boiler_min * MS_Var.Boiler_Q_max:  # Boiler can be activated?
                        ...
                        E_gas_Boiler_W = Q_primary
                        E_BaseBoiler_req_W = E_aux_Boiler
            ...
            if current_source == 'BoilerPeak' and Q_therm_req_W > 0:
                ...
                if (MS_Var.BoilerPeak_on) == 1:
                    ...
                    if Q_therm_req_W > 0:  # gv.Boiler_min*MS_Var.BoilerPeak_Q_max: # Boiler can be activated?
                        ...
                        E_gas_Backup_W = Q_primaryP
                        E_BackupBoiler_req_W = E_aux_BoilerP
    ...
    if save_file == 1:
        results = pd.DataFrame({
            "Q_Network_Demand_after_Storage": Q_missing_copy_W,
    ...
    results = pd.DataFrame({
        "EgasPrimary": [E_gas_Primary_W],
        "EgasPrimaryPeakPower": [E_gas_PrimaryPeakPower_W],
        "EwoodPrimary": [E_wood_Primary_W],
        "EelectrImportSlave": [E_Import_Slave_req_W],
        "EelExport": [E_Export_gen_W],
        "Egroundheat": [E_groundheat_W],
        "EsolarUsed": [E_solar_gen_Wh],
...
def calc_primary_energy_and_CO2(Q_source_data_W, Q_coldsource_data_W, E_PP_el_data_W,
    ...
    Eprim_from_Sewage = np.sum(Q_HPSew_gen_W) / COP_HPSew_avg  * gv.SEWAGEHP_TO_OIL_STD * gv.Wh_to_J / 1.0E6
    Eprim_from_GHP    = np.sum(Q_GHP_gen_W) / COP_GHP_avg * gv.GHP_TO_OIL_STD  * gv.Wh_to_J / 1.0E6
    Eprim_from_HPLake = np.sum(Q_HPLake_gen_W) / COP_HPLake_avg * gv.LAKEHP_TO_OIL_STD  * gv.Wh_to_J / 1.0E6
    ...
    Eprim_from_wood     = 1 /eta_furnace_avg * np.sum(Q_Furnace_gen_W) * gv.FURNACE_TO_OIL_STD * gv.Wh_to_J / 1.0E6

    EprimSaved_from_elec_sold_Furnace = np.sum(E_Furnace_gen_W) * (- EL_TO_OIL_EQ) * gv.Wh_to_J / 1.0E6
    EprimSaved_from_elec_sold_CHP     = np.sum(E_CC_gen_W) * (- EL_TO_OIL_EQ) * gv.Wh_to_J / 1.0E6
    EprimSaved_from_elec_sold_Solar   = E_solar_gen_Wh * (gv.EL_PV_TO_OIL_EQ - EL_TO_OIL_EQ) * gv.Wh_to_J / 1.0E6
    ...
    Eprim_from_SCandPVT = Q_SCandPVT_gen_Wh * gv.SOLARCOLLECTORS_TO_OIL * gv.Wh_to_J / 1.0E6
