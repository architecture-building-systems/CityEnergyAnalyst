...
def find_minimum_cost(Q_therm_req, opt_round, used_source, HOUR):
    ...
    if (MS_Var.GHP_on) == 1 and used_source != 'GHP' and HOUR > MS_Var.GHP_SEASON_ON and  HOUR <= MS_Var.GHP_SEASON_OFF:
...
Q_remaining1 = 0
used_sources = 0
def PP_Activator(Q_therm_req, opt_round, Q_provided1, Q_remaining1, used_sources, HOUR):
    
    Q_provided0, Cmarg_used0, Ctot_used0, Q_remaining0, end_iteration, used_source0, min_source0 \
                    = find_minimum_cost(Q_therm_req, opt_round, used_sources, HOUR)
    ...
    if Q_remaining0 > 0:
        ...
        Q_provided1, Cmarg_used1, Ctot_used1, Q_remaining1, end_iteration, used_source1, min_source1 \
                        = find_minimum_cost(Q_remaining0, opt_round, used_source0, HOUR)
        
        if used_source1 == 'DH_Requirement': # there is a lack of thermal energy if one tries to combine them, avoid first generation method.
            Q_provided0, Cmarg_used0, Ctot_used0, Q_remaining0, end_iteration, used_source0, min_source0 = [0,0,0,0,0,used_source0,0]
            opt_round = 1
            Q_provided1, Cmarg_used1, Ctot_used1, Q_remaining1, end_iteration, used_source1, min_source1 \
                        = find_minimum_cost(Q_therm_req, opt_round, used_source0, HOUR)
