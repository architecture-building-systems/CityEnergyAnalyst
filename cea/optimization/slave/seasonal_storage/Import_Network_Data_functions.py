...
def import_solar_thermal_data(fName):
    ...
    if fName == "PVT_total.csv":
        ...
        Solar_Tscr_th_K = np.array(solar_data['T_PVT_re_C']) + KELVIN_OFFSET
        ...
        for i in range(HOURS_IN_YEAR):
            if Solar_Q_th_kWh[i] < 0:
                Solar_Q_th_kWh[i] = 0
                Solar_E_aux_kWh[i] = 0
                Solar_Tscr_th_K[0] = Tscs + KELVIN_OFFSET
        ...
        Solar_Tscr_th_K = np.array(solar_data['T_SC_re_C']) + KELVIN_OFFSET
        Solar_Tscs_th = np.zeros(HOURS_IN_YEAR)
