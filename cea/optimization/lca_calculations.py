...
class LcaCalculations(object):
    def __init__(self, supply_systems):
        ...
        self.NG_TO_CO2_EQ = np.tile(feedstocks['NATURALGAS']['GHG_kgCO2MJ'].values,
                                    365)  # in kgCo2/MJ for every hour of a year
        self.WETBIOMASS_TO_CO2_EQ = np.tile(feedstocks['WETBIOMASS']['GHG_kgCO2MJ'].values,
                                            365)  # in kgCo2/MJ for every hour of a year
        self.DRYBIOMASS_TO_CO2_EQ = np.tile(feedstocks['DRYBIOMASS']['GHG_kgCO2MJ'].values,
                                            365)  # in kgCo2/MJ for every hour of a year
