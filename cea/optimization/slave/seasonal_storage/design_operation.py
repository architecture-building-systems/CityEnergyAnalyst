...
def Storage_Design(T_storage_old_K, Q_in_storage_old_W, locator,
    ...
    T_amb_K = weather_data['drybulb_C'] + KELVIN_CONVERSION  # K
    ...
    for HOUR in range(HOURS_IN_YEAR):  # no idea why, but if we try a for loop it does not work
        ...
        # scale to the fraction that goes directly to the grid. the rest is considered in the storage charging and discharging
