...
class GraphsDemandTool(object):
    ...
    def getParameterInfo(self):
        scenario_path = arcpy.Parameter(
            ...
            parameterType="Required",
            direction="Input")
        analysis_scemario = arcpy.Parameter(
            displayName="Scenarios to analyse",
            name="analysis_scenarios",
            datatype="String",
            parameterType="Required",
            multiValue=True,
            direction="Input")
        analysis_scenarios.filter.list = []
        return [scenario_path, analysis_scenarios]
    ...
    def updateParameters(self, parameters):
        ...
        # path_variables
        locator = inputlocator.InputLocator(scenario_path)
        scenario_names = [name for name in os.listdir(scenario_path) if os.path.isdir(os.path.join(scenario_path,name))]
        analysis_scenarios.filter.list = list(scenario_names)
        return

    def execute(self, parameters, messages):
        scenario_path = parameters[0].valueAsText
        analysis_scenarios = parameters[1].valueAsText.split(';')
...
class GraphsBenchmarkTool(object):
    ...
    def getParameterInfo(self):
        ...
        analysis_fields.parameterDependencies = ['scenario_path']
