...
results = pd.DataFrame({"Q_missing_safe":Q_missing_copy[:,0]})
Name = "PP_Activation_Results_testfile.csv"
os.chdir(Slave_PP_Activator_Results)
results.to_csv(Name, sep= ',')
print "results saved in : ", Slave_PP_Activator_Results
print " as : ", Name
    ...
    while Q_therm_req > 0: # cover dem as long as the supply is lower than dem!
        ...
        if current_source == 'HP':# use heat pumps available!
            ...
            if (MS_Var.HP_Sew_on) == 1 and Q_therm_req  > 0: # activate if its available
                ...
                #print Q_HPSew_therm, "Q_HPSew_therm", C_HPSew_el_pure, "C_HPSew_el_pure"
            ...
            #if Q_therm_req == 0: # all the dem has been covered already, finish the loop
                #continue
                #break
                #print hour
            ...
            if (MS_Var.GHP_on) == 1 and hour > MS_Var.GHP_SEASON_ON and  hour <= MS_Var.GHP_SEASON_OFF and Q_therm_req > 0: # activating GHP plant if possible

 #               Q_req =  mdot_DH_req * gV.cp*(tdhsup -tdhret_req)
                ...
                #if Q_therm_req > MS_Var.GHP_max  : # 100% load, cannot go above (scale down the dem)
                ...
                #    mdot_DH_to_GHP = mdot_DH_req * Q_therm_GHP / Q_therm_req #scale down the mass flow if the thermal dem is lowered
                    ...
                    mdot_DH_to_GHP = Q_therm_req / (gV.cp * (tdhsup -tdhret_req))
                    Q_therm_req = 0
        ...
        if np.floor(Q_therm_req) > 0:
            if current_source == gV.act_first:
                ...
                print Q_therm_req
                #print "Power Plant Status: "
                #print "sHPSew, sHPLake,  srcGHP,  sorcCC,  sBoiler,  sBackup :", sHPSew, sHPLake,  srcGHP,  sorcCC,  sBoiler,  sBackup
                #raise MS_VarError
                
 
        else:#elif Q_therm_req != 0:
            print "ERROR - TOO MUCH POWER! - ", -Q_therm_req/1000.0,"kWh in excess"
        #if checkpoint == 1:
        #    print "All dem covered"
        #else:
        #    print Q_remaining, "Wh not covered" 
    #print Q_remaining, "Wh not covered after completion"
    #print "last source used: ", current_source 
...
#used_sources_res = np.zeros((gV.HOURS_IN_DAY * gV.DAYS_IN_YEAR,7)) #np.chararray((8760,2), itemsize=5)
...
for hour in range(gV.HOURS_IN_DAY * gV.DAYS_IN_YEAR):
    ...
    #print Q_therm_req - Q_missing_copy[hour], hour
# save data 
...
save_file = 1
if save_file == 1:
    results = pd.DataFrame({"Q_missing_TEST":Q_missing_copy[:,0], "Q_missing_used": Q_missing[:,0],\
                            "Total_Cost_HPSew":cost_data[:,0],"Total_Cost_HPLake":cost_data[:,1],"Total_Cost_GHP":cost_data[:,2],\
                            "Total_Cost_CC":cost_data[:,3], "Total_Cost_Furnace":cost_data[:,5],"Total_Cost_Boiler":cost_data[:,5],\
...
if MS_Var.gt_fuel == 'NG': # Differenciate between NG and BG as input
