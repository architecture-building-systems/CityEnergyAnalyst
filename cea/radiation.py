...
def solar_radiation_vertical(locator, path_arcgis_db, latitude, longitude, timezone, year):
    ...
    # Select buildings
    buildings_selection = path_arcgis_db + '\\' + 'building_selection'
    ...
    arcpy.SimplifyBuilding_cartography(locator.get_building_geometry(), Simple_context, simplification_tolerance=8,
                                       minimum_area=None)

    # burn buildings into raster
    burn_buildings_into_raster(Simple_context, locator.get_terrain(), dem_rasterfinal, locator.get_temporary_folder(),
                               dem_raster_extent)

    # Calculate boundaries of buildings
    CalcBoundaries(Simple_CQ, locator.get_temporary_folder(), path_arcgis_db, DataFactorsCentroids,
                   DataFactorsBoundaries)

    # calculate observers
    CalcObservers(Simple_CQ, observers, DataFactorsBoundaries, path_arcgis_db)
        ...
        while result == None:
            try:
                result = CalcRadiation(day, dem_rasterfinal, observers, T_G_day, latitude,
                                       locator.get_temporary_folder(), aspect_slope, heightoffset)
            except:
                pass
    ...
    CalcRadiationSurfaces(observers, locator.get_radiation(), DataFactorsCentroids,
...
# Functions
def CalcRadiationSurfaces(Observers, Radiationyearfinal, DataFactorsCentroids, DataradiationLocation, locationtemp1,
                          locationtemp2):
    # local variables
...
def CalcRadiation(day, memoryFeature, Observers, T_G_day, latitude, locationtemp1, aspect_slope, height_offset):
    # Local Variables
    ...
    transmissivity = str(T_G_day.loc[day - 1, 'ttr'])
    height_offset = str(height_offset)
    global_radiation = locationtemp1 + '\\' + 'Day_' + str(day) + '.shp'
    timeConfig = 'WithinDay    ' + str(day) + ', 0, 24'

    # Run the extension of arcgis
    arcpy.gp.PointsSolarRadiation_sa(memoryFeature, Observers, global_radiation, height_offset,
                                     Latitude, skySize, timeConfig, dayInterval, hourInterval, "INTERVAL", "1",
                                     aspect_slope, calcDirections, zenithDivisions, azimuthDivisions,
                                     "STANDARD_OVERCAST_SKY", diffuseProp, transmissivity, "#", "#", "#")
...
def CalcObservers(Simple_CQ, Observers, DataFactorsBoundaries, locationtemporal2):
    ...
    # Import Overlapable from function CalcBoundaries containing the data about buildings overlaping,
    # eliminate duplicates, chose only those ones no overlaped and reindex
    ...
    for row in range(rows):
        ...
        where_clause = '''''' + '"' + Field + '"' + "=" + "\'" + str(
            Value) + "\'" + ''''''  # strange writing to introduce in ArcGIS
        if row == 0:
            arcpy.MakeFeatureLayer_management(Simple_CQ, 'Simple_lyr')
            arcpy.SelectLayerByAttribute_management('Simple_lyr', "NEW_SELECTION", where_clause)
        else:
            arcpy.SelectLayerByAttribute_management('Simple_lyr', "ADD_TO_SELECTION", where_clause)
...
def burn_buildings_into_raster(Buildings, DEM, DEMfinal, locationtemp1, DEM_extent):
...
def test_solar_radiation():
    ...
    solar_radiation_vertical(locator=locator, path_arcgis_db=path_default_arcgis_db,
                             latitude=46.95240555555556, longitude=7.439583333333333, timezone=1, year=2014)
