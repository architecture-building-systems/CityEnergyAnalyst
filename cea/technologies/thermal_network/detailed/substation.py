...
from cea.constants import (HEAT_CAPACITY_OF_WATER_JPERKGK, P_WATER_KGPERM3,
                           MIN_TEMP_DIFF_FOR_MASS_FLOW_K, MIN_TEMP_DIFF_FOR_HEX_LMTD_K, KELVIN_OFFSET)
...
def substation_HEX_sizing(building_demand, substation_systems, thermal_network):
    ...
    # Series for storage
    hex_areas = pd.Series(index=area_columns, dtype=float)      # Series 1
    UA_data = pd.Series(index=UA_columns, dtype=float)          # Series 2
    Q_nom_data = pd.Series(index=Q_columns, dtype=float)        # Series 3
...
def calc_substation_return_DH(building, T_DH_supply_K, substation_HEX_specs, thermal_network, name, t):
    ...
    if 'UA_heating_hs_ahu' in substation_HEX_specs.HEX_UA:
        ...
        T_DH_supply_C = T_DH_supply_K - KELVIN_OFFSET  # Convert K to C
        ...
        if T_DH_supply_C < (T_hs_target_C[0] + MIN_APPROACH_TEMP_K):
            ...
            t_DH_return_hs_ahu = T_dh_return_C_arr[0] + KELVIN_OFFSET  # C to K
    ...
    if 'UA_heating_hs_aru' in substation_HEX_specs.HEX_UA:
        ...
        if T_DH_supply_C < (T_hs_target_C[0] + MIN_APPROACH_TEMP_K):
            ...
            # Extract scalar values from arrays
            t_DH_return_hs_aru = T_dh_return_C_arr[0] + KELVIN_OFFSET  # C to K
    ...
    if 'UA_heating_hs_shu' in substation_HEX_specs.HEX_UA:
        ...
        if T_DH_supply_C < (T_hs_target_C[0] + MIN_APPROACH_TEMP_K):
            ...
            # Extract scalar values from arrays
            t_DH_return_hs_shu = T_dh_return_C_arr[0] + KELVIN_OFFSET  # C to K
    ...
    if 'UA_heating_hs_ww' in substation_HEX_specs.HEX_UA:
        ...
        if T_DH_supply_C < (T_ww_target_C[0] + MIN_APPROACH_TEMP_K):
            ...
            # Extract scalar values from arrays
            t_DH_return_ww = T_dh_return_C_arr[0] + KELVIN_OFFSET  # C to K
...
def calc_substation_return_DC(building, T_DC_supply_K, substation_HEX_specs, thermal_network, name, t):
    ...
    # Cooling ahu
    if 'UA_cooling_cs_ahu' in substation_HEX_specs.HEX_UA:
    ...
    # Cooling aru
    if 'UA_cooling_cs_aru' in substation_HEX_specs.HEX_UA:
    ...
    # Cooling scu
    if 'UA_cooling_cs_scu' in substation_HEX_specs.HEX_UA:
    ...
    if 'UA_cooling_cs_data' in substation_HEX_specs.HEX_UA:
    ...
    if 'UA_cooling_cs_re' in substation_HEX_specs.HEX_UA:
...
def main(config: cea.config.Configuration):
    ...
    T_substation_supply_K = pd.DataFrame([[T_DH + KELVIN_OFFSET] * len(thermal_network.building_names)],
