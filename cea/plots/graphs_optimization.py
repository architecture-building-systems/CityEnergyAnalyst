...
def plot_pareto_scenarios(locator, generations, relative):
    ...
    pop, eps, testedPop, ntwList = sFn.readCheckPoint(locator, generations, 0)
...
def plot_buildings_connected_scenarios(scenarios, generations, headers):
    ...
    for header in headers:
        ...
        for i in range(generations[counter]):
            i += 1
            pop, eps, testedPop, ntwList = sFn.readCheckPoint(pathX, i, 0)
            buildCon = []
            buildCon2 = []
            for ind in pop:
...
def buildingConnection(generation, locator):
    ...
    for i in range(generation):
        i += 1
        pop, eps, testedPop, ntwList = sFn.readCheckPoint(locator, i, 0)
        buildCon = []
