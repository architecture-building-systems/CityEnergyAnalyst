...
def pvt_district_monthly(data_frame, analysis_fields, title, output_path):
    ...
    E_analysis_fields_used = data_frame.columns[data_frame.columns.isin(analysis_fields)].tolist()
    Q_analysis_fields_used = data_frame.columns[data_frame.columns.isin(analysis_fields)].tolist()

    #CALCULATE GRAPH
    traces_graph = calc_graph(E_analysis_fields_used, Q_analysis_fields_used, data_frame)
    ...
    layout = go.Layout(images=LOGO,title=title, barmode='stack', yaxis=dict(title='PVT Electricity/Heat production [MWh]',
                                                                            domain=[0.35, 1]))
...
def calc_graph(E_analysis_fields_used, Q_analysis_fields_used, data_frame):
    ...
    for field in E_analysis_fields_used:
        ...
        trace = go.Bar(x=new_data_frame["month"], y=y, name=field.split('_E_kWh',1)[0], text = total_perc_txt,
                       marker=dict(color=COLOR[field.split('_E_kWh',1)[0].split('PVT_',1)[1]]))
        graph.append(trace)
    ...
    for field in Q_analysis_fields_used:
        ...
        total_perc_txt = ["("+str(x)+" %)" for x in total_perc]
        trace = go.Bar(x=new_data_frame["month"], y=y, name=field.split('_Q_kWh',1)[0], text = total_perc_txt,
                       marker=dict(color=COLOR[field.split('_Q_kWh',1)[0].split('PVT_',1)[1]]))
        graph.append(trace)
