...
def calc_table(analysis_fields, data_frame):
    ...
    data_frame = data_frame.join(total)  # join dataframes
    anchors = []
    load_names = []
    median = []
    peak = []
    ...
    for field in analysis_fields:
        ...
        load_names.append(NAMING[field.split('_', 1)[0]] + ' (' + field.split('_', 1)[0] + ')')  # get correct name
    ...
    table = go.Table(domain=dict(x=[0, 1.0], y=[0, 0.2]),
                     header=dict(
                         values=['Load Name', 'Total [kWh/yr]', 'Peak [kW]', 'Median [kWh]', 'Top 3 Consumers']),
...
def calc_graph(analysis_fields, data_frame):
    ...
    for field in analysis_fields:
        ...
        trace = go.Bar(x=data_frame.index, y=data_frame[field].values, name=field.split('_', 1)[0], text=total_perc_txt,
                       orientation='v',
