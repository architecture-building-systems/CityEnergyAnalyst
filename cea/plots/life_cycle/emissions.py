...
class EmissionsPlot(cea.plots.life_cycle.LifeCycleAnalysisPlotBase):
    """Implements the Green House Gas Emissions plot"""
    ...
    def calc_graph(self):
        # calculate graph
        graph = []
        analysis_fields = self.analysis_fields_emissions
    ...
    def calc_table(self):
        ...
        column_names = ['Category', 'Emissions for all buildings [ton CO2-eq/yr]', 'Top 3 most polluting buildings']
...
def main():
    ...
    EmissionsPlot(config.project, {'buildings': None,
                                   'scenario-name': config.scenario_name},
                  cache).plot(auto_open=True)
    EmissionsPlot(config.project, {'buildings': locator.get_zone_building_names()[0:2],
                                   'scenario-name': config.scenario_name},
                  cache).plot(auto_open=True)
    EmissionsPlot(config.project, {'buildings': [locator.get_zone_building_names()[0]],
                                   'scenario-name': config.scenario_name},
                  cache).plot(auto_open=True)
