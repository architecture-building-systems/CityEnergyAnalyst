...
TSD_COLUMNS = ['T_ext', 'rh_ext', 'w_int', 'I_sol_and_I_rad', 'Q_gain_lat_peop', 'Q_gain_sen_peop', 'Q_gain_sen_vent',
...
def main():
    ...
    for name in building_names:
        ...
        ## CO2 gain
        calc_CO2_gains(demand_df, output_df, reduced_tsd_df)

        # exhaust rate from SS553
        output_df['m_exhaust_from_SS553'] = Af_m2[name] * (SS553_lps_m2 / 1000) * calc_rho_air(24)  # TODO: CHECK CALC
        ...
        output_df = output_df.drop(output_df.index[range(8)])
        output_df.T.ix[1:].to_csv(path_to_osmose_project(name), header=False)
    return


def calc_CO2_gains(demand_df, output_df, reduced_tsd_df):
    ...
    output_df['v_CO2_infil_window_m3pers'] = reduced_tsd_df['v_in_infil_window'] * output_df['CO2_ext_ppm']
    output_df['v_CO2_in_infil_occupant_m3pers'] = output_df['v_CO2_occupant_m3pers'] + output_df[
        'v_CO2_infil_window_m3pers']

    # output_df['v_CO2_infil_window_m3perhr'] = reduced_tsd_df['v_in_infil_window']*3600*output_df['CO2_ext_ppm']
    # output_df['v_CO2_in_infil_occupant_m3perhr'] = output_df['v_CO2_occupant_m3perhr'] + output_df['v_CO2_infil_window_m3perhr'] #to hr
    output_df['m_exhaust_from_CO2'] = np.vectorize(calc_m_exhaust_from_CO2)(output_df['CO2_max_ppm'],
                                                                            output_df['CO2_ext_ppm'],
                                                                            output_df['v_CO2_occupant_m3pers'],
                                                                            reduced_tsd_df['rho_air'])


def calc_sensible_gains(output_df, reduced_tsd_df):
    # radiation
    output_df['Q_gain_rad_kWh'] = reduced_tsd_df['I_sol_and_I_rad'] / 1000
    # environment
    output_df['Q_gain_env_kWh'] = reduced_tsd_df.loc[:, Q_GAIN_ENV].sum(axis=1) / 1000
    # internal (appliances, lighting...)
    output_df['Q_gain_int_kWh'] = reduced_tsd_df.loc[:, Q_GAIN_INT].sum(axis=1) / 1000
    # occupant
    output_df['Q_gain_occ_kWh'] = reduced_tsd_df['Q_gain_sen_peop'] / 1000
    # total
    output_df['Q_gain_total_kWh'] = output_df['Q_gain_rad_kWh'] + output_df['Q_gain_env_kWh'] + output_df[
        'Q_gain_int_kWh'] + output_df['Q_gain_occ_kWh']
