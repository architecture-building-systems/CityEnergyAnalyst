...
import numpy as np
import pandas as pd
import substation_combined_functions_V3_NewName as fn
reload(fn)
...
T_supply_max_all_buildings = np.array(fn.find_max_temp_of_buildings(building_list, "tsh", gV.DAYS_IN_YEAR, gV.HOURS_IN_DAY))
...
for i in range(len(building_list_array)):
    ...
    T_combined_max_all_buildings_flat = np.zeros(gV.DAYS_IN_YEAR*gV.HOURS_IN_DAY)
    
    for hour in range(gV.DAYS_IN_YEAR*gV.HOURS_IN_DAY):
        T_combined_max_all_buildings_flat[hour] = max(T_hotwater_max_all_buildings_flat[hour],T_supply_max_all_buildings_flat[hour])
    ...
    results = pd.DataFrame({"mdot_DH_result":mdot_DH_result_flat,"T_return_DH_result":T_return_DH_result_flat,\
                ...
                "Q_cool":Q_cool_array_flat, "T_heating_supply_max_all_buildings":T_supply_max_all_buildings_flat,\
                "T_hotwater_max_all_buildings":T_hotwater_max_all_buildings_flat, \
                "T_combined_max_all_buildings":T_combined_max_all_buildings_flat})
