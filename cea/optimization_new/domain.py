...
class Domain(object):
        ...
        try:
            ...
            if not network_name or not network_name.strip():
                raise UserNetworkLoaderError(
                    "No base network specified for optimisation.\n\n"
    ...
    def optimize_domain(self):
        ...
        if not buildings_with_supply and not buildings_designated_for_networks:
            ...
            if total_demand > 0:
                ...
                raise ValueError(
                    ...
                    "Solutions:\n"
                    "  1. Update Input Editor > supply to use valid assemblies for buildings with demand:\n"
        ...
        if buildings_without_supply:
            ...
            print("    → These buildings will be network-supplied if connected during optimisation")
...
def main(config: cea.config.Configuration):
    ...
    if config.optimization_new.network_name:
        ...
        # Autogenerated layout mode - Input Editor > supply settings determines connectivity
        print("\nUsing auto-generated network layout (connectivity from Input Editor > supply settings DISTRICT scale)")
    ...
    if config.optimization_new.network_name and hasattr(current_domain, 'base_building_to_network'):
        ...
        # VALIDATION 2: Check if Input Editor > supply settings matches network layout
        print("\nValidating Input Editor > supply settings against network layout:")
        mismatches = []
        ...
        for building in current_domain.buildings:
            ...
            if in_network and not supply_is_district:
                # Building is in network but Input Editor > supply settings has BUILDING scale
                mismatches.append({
                ...
                # Building is NOT in network but Input Editor > supply settings has DISTRICT scale
                mismatches.append({
                    'building': building_id,
        ...
        if mismatches:
            ...
            error_msg = (
                f"\n\n✗ ERROR: Input Editor > supply settings do not match network layout '{config.optimization_new.network_name}'.\n\n"
                "Buildings IN network must have DISTRICT scale in Input Editor > supply settings:\n"
                f"  - In network: {sorted(network_buildings)}\n\n"
                "Buildings NOT in network must have BUILDING scale in Input Editor > supply settings:\n"
                f"  - Not in network: {sorted(loaded_buildings - network_buildings)}\n\n"
                "Please update Input Editor > supply settings to match the network layout."
            )
            raise ValueError(error_msg)

        print("✓ All buildings have matching Input Editor > supply settings")
    ...
    # If using auto-generated layout, just report connectivity from Input Editor > supply settings
    ...
    if not config.optimization_new.network_name:
        print("\nConnectivity determined from Input Editor > supply settings; layout will be auto-generated:")
        ...
        if network_buildings:
            print(f"  Buildings for auto-generated networks (DISTRICT scale in Input Editor > supply settings): {network_buildings}")
            print("    → Network layout will be generated using Steiner tree algorithm on street network")
        if standalone_buildings:
            print(f"  Standalone buildings (BUILDING scale in Input Editor > supply settings): {standalone_buildings}")
        print()
