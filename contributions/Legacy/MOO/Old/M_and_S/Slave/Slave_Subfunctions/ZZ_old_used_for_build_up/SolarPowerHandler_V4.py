...
import Storage_Optimizer as StOp
reload(StOp)
reload(MS_Var)
os.chdir(Energy_Models_path)
reload (gV)
...
def Storage_Charger(T_storage_old, Q_to_storage_lossfree, T_DH_ret, Q_in_storage_old):
    ...
    T_storage_new = MS_Var.T_storage_zero + Q_in_storage_new * gV.Wh_to_J / (float(MS_Var.STORAGE_SIZE) * float(gV.cp) * float(gV.rho_60))
...
def Storage_Operator(Q_solar_available, Q_network_demand, T_storage_old, T_DH_sup, T_amb, Q_in_storage_old, T_DH_return, mdot_DH):
    ...
    if to_storage == 1: # charging the storage
        ...
        T_storage_new, Q_to_storage_new, Q_aux_ch, Q_in_storage_new = Storage_Charger(T_storage_old, Q_to_storage, T_DH_return, Q_in_storage_old)
        ...
        if Q_in_storage_old > 0:
            Q_aux_dech, Q_from_storage_used, Q_in_storage_new, T_storage_new, COP = Storage_DeCharger(T_storage_old, Q_from_storage_req, T_DH_sup, Q_in_storage_old)
            ...
            if Q_in_storage_new < 0: # if storage is almost empty, to not go below 10 degC, just do not provide more energy than possible.
                ...
                Q_aux_dech, Q_from_storage_used, Q_in_storage_new, T_storage_new, COP = Storage_DeCharger(T_storage_old, Q_from_storage_poss, T_DH_sup, Q_in_storage_old)
    ...
    return Q_in_storage_new, T_storage_new, Q_from_storage_req, Q_to_storage, Q_aux_ch, Q_aux_dech, Q_missing, Q_from_storage_used
