...
def calc_Ctot_pump_netw(ntwFeat, gv, locator, prices, config):
    ...
    for network_name in network_names:
        ...
        df = pd.read_csv(locator.get_node_mass_flow_csv_file(network_type, network_name), index_col=0)
        ...
        deltaP_kW = pd.read_csv(locator.get_ploss(network_name, network_type))
...
def main(config):
    ...
    for building_name in building_names:
        ...
        Cost_storage['Capex_total'][building_name] = Capex_a_netw + Capex_a_pump
        Cost_storage['Opex_total'][building_name] = Opex_fixed_pump + Opex_heat
        Cost_storage['Cost_total'][building_name] = Cost_storage['Capex_total'][building_name] + \
