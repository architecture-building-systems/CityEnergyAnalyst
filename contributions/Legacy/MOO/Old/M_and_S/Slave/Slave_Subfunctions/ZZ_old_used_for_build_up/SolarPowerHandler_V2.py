...
def Storage_Charger(T_storage_old, Q_to_storage_lossfree, T_DH_ret, Q_storage_old):
    """
...
def Storage_Operator(Q_solar_available, Q_network_demand, T_storage_old, T_DH_sup, T_amb, Q_in_storage_old, T_DH_return, mdot_DH):
    ...
    Q_to_storage_lossfree, Q_from_storage_req, to_storage = StorageGateway(Q_solar_available, Q_network_demand)
    Q_missing = 0
    
    T_before_PP = T_DH_sup
    
    if to_storage == 1: # charging the storage
        T_storage_new, Q_to_storage_new, Q_aux, Q_storage_new = Storage_Charger(T_storage_old, Q_to_storage_lossfree, T_DH_return, Q_in_storage_old)
        
        Q_to_storage = Q_to_storage_new  # including losses!
