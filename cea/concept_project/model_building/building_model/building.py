...
class Building(object):
    ...
    def __init__(self, conn, scenario_name):
        ...
        # CO2 model or ahu or windows
        self.define_constant = ((self.building_scenarios['co2_model_type'][0] != '')
                                | (self.building_zones['hvac_ahu_type'] != '').any()
                                | (self.building_zones['window_type'] != '').any())

        # Define model dimensions
        self.n_states = (
                self.building_zones.shape[0]
                + sum(self.parse_parameter(row['heat_capacity']) != 0
        ...
        )  # Temperatures of all zones and surfaces + co2 concentrations of zones with ventilation
        # + absolute humidity of zones with ahu
        self.n_zones_co2 = \
            (((self.building_zones["hvac_ahu_type"] != "") | (self.building_zones['window_type'] != ''))
             & (self.building_scenarios['co2_model_type'][0] != "")
             ).sum()
        self.n_zones_humidity = \
            ((self.building_zones['hvac_ahu_type'] != '')
             & (self.building_scenarios['humidity_model_type'][0] != '')
             ).sum()
        self.n_zones = self.building_zones.shape[0]
        self.n_zones_ventilation = \
            ((self.building_zones["hvac_ahu_type"] != "") | (self.building_zones['window_type'] != '')
             ).sum()
        self.n_controls = (
                (self.building_zones['window_type'] != '').sum()
                + (self.building_zones['hvac_generic_type'] != '').sum() * 2
                + (self.building_zones['hvac_ahu_type'] != '').sum() * 2
                + (self.building_zones['hvac_tu_type'] != '').sum() * 2
        )  # Zones with HVAC systems (heating + cooling) and zones that can be ventilated
        self.n_disturbances = (
                7
                + self.building_zones['internal_gain_type'].unique().shape[0] * 2
                + (1 if self.define_constant else 0)
        )  # Ambient air temperature, sky temperature, irradiation, occupancy, appliances, constant term
        self.n_outputs = (
                self.building_zones.shape[0]
                + (((self.building_zones['hvac_ahu_type'] != '') | (self.building_zones['window_type'] != ''))
                   & (self.building_scenarios['co2_model_type'][0] != '')
                ...
                + (self.building_zones['hvac_ahu_type'] != '').sum()
                + (self.building_zones['window_type'] != '').sum()
                + (self.building_zones['hvac_generic_type'] != '').sum() * 2
                + (self.building_zones['hvac_ahu_type'] != '').sum() * 2
                + (self.building_zones['hvac_tu_type'] != '').sum() * 2
        )  # Zone temperatures of all zones, co2 concentrations of zones with AHU or windows, humidity of zones with ahu
        # fresh air flow of zones that can be ventilated, fresh air flow from ahu, fresh air flow from windows,
        # (electric) power of HVAC system
        self.n_outputs_control = (
                (self.building_zones['hvac_generic_type'] != '').sum() * 2
                + (self.building_zones['hvac_ahu_type'] != '').sum() * 2
                + (self.building_zones['hvac_tu_type'] != '').sum() * 2
        ...
        self.i_controls = pd.Series(
            ...
            pd.concat([
                ...
                self.building_zones['zone_name'][
                    self.building_zones['hvac_ahu_type'] != ''
                    ] + '_ahu_heat_air_volume_flow',
                self.building_zones['zone_name'][
                    self.building_zones['hvac_ahu_type'] != ''
                    ] + '_ahu_cool_air_volume_flow',
                self.building_zones['zone_name'][
                    self.building_zones['hvac_tu_type'] != ''
                    ] + '_tu_heat_air_volume_flow',
                self.building_zones['zone_name'][
                    self.building_zones['hvac_tu_type'] != ''
                    ] + '_tu_cool_air_volume_flow',
                self.building_zones['zone_name'][
                    (self.building_zones['window_type'] != '')
                    ] + '_window_air_flow'
            ])
        )
        self.i_disturbances = pd.Series(
            np.arange(self.n_disturbances),
            pd.concat([
                pd.Series([
        ...
        self.i_outputs = pd.Series(
            np.arange(self.n_outputs),
            pd.concat([
                self.building_zones['zone_name'] + '_temperature',
                self.building_zones['zone_name'][
                    ((self.building_zones['hvac_ahu_type'] != '') | (self.building_zones['window_type'] != ''))
        ...
        self.i_zones_co2 = pd.Series(
            np.arange(self.n_zones_co2),
            self.building_zones['zone_name'][
                ((self.building_zones['hvac_ahu_type'] != '') | (self.building_zones['window_type'] != ''))
                & (self.building_scenarios['co2_model_type'][0] != '')
            ]
        )
        self.i_zones_humidity = pd.Series(
            np.arange(self.n_zones_humidity),
            self.building_zones['zone_name'][
                (self.building_zones['hvac_ahu_type'] != '') & (self.building_scenarios['humidity_model_type'][0] != '')
            ]
        )
        self.i_zones_ventilation = pd.Series(
            np.arange(self.n_zones_ventilation),
            self.building_zones['zone_name'][
                ((self.building_zones['hvac_ahu_type'] != '') | (self.building_zones['window_type'] != ''))]
        )
        self.i_zones = pd.Series(
            np.arange(self.n_zones),
            self.building_zones['zone_name']
        )
        self.i_outputs_control = pd.Series(
            np.arange(self.n_outputs_control),
            pd.concat([self.building_zones['zone_name'][
                    self.building_zones['hvac_generic_type'] != ''
                ] + '_generic_heat_power',
                self.building_zones['zone_name'][
                    self.building_zones['hvac_generic_type'] != ''
        ...
        self.control_matrix = pd.DataFrame(
            ...
            self.i_states.index,
            self.i_controls.index
        )
        self.disturbance_matrix = pd.DataFrame(
            np.zeros((self.n_states, self.n_disturbances)),
            self.i_states.index,
            self.i_disturbances.index
        )
        self.state_output_matrix = pd.DataFrame(
            np.zeros((self.n_outputs, self.n_states)),
            self.i_outputs.index,
            self.i_states.index
        )
        self.control_output_matrix = pd.DataFrame(
            np.zeros((self.n_outputs, self.n_controls)),
            self.i_outputs.index,
            self.i_controls.index
        )
        self.disturbance_output_matrix = pd.DataFrame(
            np.zeros((self.n_outputs, self.n_disturbances)),
            self.i_outputs.index,
            self.i_disturbances.index
        )
        ...
        self.n_time = self.time_vector.shape[0]
        self.i_time = pd.Series(
            np.arange(self.n_time),
            self.time_vector
        )
    ...
    def define_heat_transfer_coefficients(self):
        ...
        for index_exterior, row_exterior in self.building_surfaces_exterior.iterrows():
            row_exterior['heat_transfer_coefficient_surface_to_sky'] = (
                    ...
                    * (self.parse_parameter(self.building_scenarios['linearization_exterior_surface_temperature']) / 2
                       + self.parse_parameter(self.building_scenarios['linearization_sky_temperature']) / 2
                       + 273.15
                       ) ** 3
            )
            row_exterior['heat_transfer_coefficient_surface_to_ground'] = (
                    ...
                    * (self.parse_parameter(self.building_scenarios['linearization_exterior_surface_temperature']) / 2
                       + self.parse_parameter(self.building_scenarios['linearization_ambient_air_temperature']) / 2
                       + 273.15
                       ) ** 3
            )
    ...
    def define_heat_transfer_adjacent_surfaces_zones(self):
        ...
        for building_surfaces in [
            ...
            for index, row in building_surfaces.iterrows():
                if self.parse_parameter(row['heat_capacity']) != 0:
                    ...
                    self.state_matrix.at[row['zone_name'] + '_temperature', index + '_temperature'] = (
                            self.state_matrix.at[row['zone_name'] + '_temperature', index + '_temperature']
                            + 1 / (
                                    1 / self.parse_parameter('heat_transfer_coefficient_surface_to_zone')
                                    + self.parse_parameter(row['thermal_resistance_surface'])
        ...
        for index, row in self.building_surfaces_interior.iterrows():
            if self.parse_parameter(row['heat_capacity']) != 0:
                self.state_matrix.at[index + '_temperature', row['zone_adjacent_name'] + '_temperature'] = (
                        self.state_matrix.at[index + '_temperature', row['zone_adjacent_name'] + '_temperature']
                        + 1 / (
                                1 / self.parse_parameter('heat_transfer_coefficient_surface_to_zone')
                                + self.parse_parameter(row['thermal_resistance_surface'])
                ...
                self.state_matrix.at[row['zone_adjacent_name'] + '_temperature', index + '_temperature'] = (
                        self.state_matrix.at[row['zone_adjacent_name'] + '_temperature', index + '_temperature']
                        + 1 / (
                                1 / self.parse_parameter('heat_transfer_coefficient_surface_to_zone')
                                + self.parse_parameter(row['thermal_resistance_surface'])
                ...
                self.state_matrix.at[
                    row['zone_adjacent_name'] + '_temperature', row['zone_adjacent_name'] + '_temperature'
                ] = (
                        self.state_matrix.at[
                            row['zone_adjacent_name'] + '_temperature', row['zone_adjacent_name'] + '_temperature'
                        ]
                ...
                self.state_matrix.at[row['zone_name'] + '_temperature', row['zone_adjacent_name'] + '_temperature'] = (
                        self.state_matrix.at[
                            row['zone_name'] + '_temperature', row['zone_adjacent_name'] + '_temperature']
                        + 1 / (
                                2 * (1 / self.parse_parameter('heat_transfer_coefficient_surface_to_zone')
                                     + self.parse_parameter(row['thermal_resistance_surface']))
                        )
                        * self.parse_parameter(row['surface_area'])
                        * (1 - self.parse_parameter(row['window_wall_ratio']))
                ...
                self.state_matrix.at[row['zone_adjacent_name'] + '_temperature', row['zone_name'] + '_temperature'] = (
                        self.state_matrix.at[
                            row['zone_adjacent_name'] + '_temperature', row['zone_name'] + '_temperature']
                        + 1 / (
                                2 * (1 / self.parse_parameter('heat_transfer_coefficient_surface_to_zone')
                                     + self.parse_parameter(row['thermal_resistance_surface']))
                        )
        ...
        for index, row in self.building_surfaces_exterior.iterrows():
            if self.parse_parameter(row['heat_capacity']) != 0:
                self.state_matrix.at[index + '_temperature', index + '_temperature'] = (
                        self.state_matrix.at[index + '_temperature', index + '_temperature']
                        - 1 / (
                                1 / self.parse_parameter('heat_transfer_coefficient_surface_to_ambient')
                                + self.parse_parameter(row['thermal_resistance_surface'])
                                * (1
                ...
                self.disturbance_matrix.at[index + '_temperature', 'ambient_air_temperature'] = (
                        self.disturbance_matrix.at[index + '_temperature', 'ambient_air_temperature']
                        + 1 / (
                                1 / self.parse_parameter('heat_transfer_coefficient_surface_to_ambient')
                                + self.parse_parameter(row['thermal_resistance_surface'])
                ...
                self.state_matrix.at[row['zone_name'] + '_temperature', row['zone_name'] + '_temperature'] = (
                        self.state_matrix.at[row['zone_name'] + '_temperature', row['zone_name'] + '_temperature']
                        - 1 / (
                                2 * self.parse_parameter(row['thermal_resistance_surface'])
                                + 1 / self.parse_parameter('heat_transfer_coefficient_surface_to_ambient')
                                + 1 / self.parse_parameter('heat_transfer_coefficient_surface_to_zone')
                                + (2 * self.parse_parameter(row['thermal_resistance_surface'])
                                   + 1 / self.parse_parameter('heat_transfer_coefficient_surface_to_zone'))
        ...
        for index, row in self.building_surfaces_exterior.iterrows():
            if self.parse_parameter(row['window_wall_ratio']) != 0:
                self.state_matrix.at[row['zone_name'] + '_temperature', row['zone_name'] + '_temperature'] = (
                        self.state_matrix.at[row['zone_name'] + '_temperature', row['zone_name'] + '_temperature']
                        - 1 / (
                                2 * self.parse_parameter(row['thermal_resistance_window'])
                                + 1 / self.parse_parameter('heat_transfer_coefficient_surface_to_ambient')
                                + 1 / self.parse_parameter('heat_transfer_coefficient_surface_to_zone')
                                + (2 * self.parse_parameter(row['thermal_resistance_window'])
                                   + 1 / self.parse_parameter('heat_transfer_coefficient_surface_to_zone'))
    ...
    def define_heat_transfer_window_air_flow(self):
        for index, row in self.building_zones.iterrows():
            if row['window_type'] != '':
                self.control_matrix.at[index + '_temperature', index + '_window_air_flow'] = (
                        self.control_matrix.at[index + '_temperature', index + '_window_air_flow']
                        + self.parse_parameter('heat_capacity_air')
    ...
    def define_heat_transfer_internal_gains(self):
        for index, row in self.building_zones.iterrows():
            self.disturbance_matrix.at[index + '_temperature', row['internal_gain_type'] + '_occupancy'] = (
                    self.disturbance_matrix.at[index + '_temperature', row['internal_gain_type'] + '_occupancy']
                    + self.parse_parameter(row['internal_gain_occupancy_factor'])
                    * self.parse_parameter(row['zone_area'])
                    / self.heat_capacity_vector[index]
            )
            self.disturbance_matrix.at[index + '_temperature', row['internal_gain_type'] + '_appliances'] = (
                    self.disturbance_matrix.at[index + '_temperature', row['internal_gain_type'] + '_appliances']
    ...
    def define_heat_transfer_hvac_generic(self):
        for index, row in self.building_zones.iterrows():
            if row['hvac_generic_type'] != '':
                self.control_matrix.at[index + '_temperature', index + '_generic_heat_thermal_power'] = (
                        self.control_matrix.at[index + '_temperature', index + '_generic_heat_thermal_power']
                        + 1
                        / self.heat_capacity_vector[index]
                )
                self.control_matrix.at[index + '_temperature', index + '_generic_cool_thermal_power'] = (
                        self.control_matrix.at[index + '_temperature', index + '_generic_cool_thermal_power']
                        - 1
                        / self.heat_capacity_vector[index]
                )
    ...
    def define_heat_transfer_hvac_ahu(self):
        for index, row in self.building_zones.iterrows():
            if row['hvac_ahu_type'] != '':
                self.control_matrix.at[index + '_temperature', index + '_ahu_heat_air_volume_flow'] = (
                        self.control_matrix.at[index + '_temperature', index + '_ahu_heat_air_volume_flow']
                        + self.parse_parameter('heat_capacity_air')
                ...
                self.control_matrix.at[index + '_temperature', index + '_ahu_cool_air_volume_flow'] = (
                        self.control_matrix.at[index + '_temperature', index + '_ahu_cool_air_volume_flow']
                        + self.parse_parameter('heat_capacity_air')
                        * (self.parse_parameter(row['ahu_supply_air_temperature_setpoint'])
                           - self.parse_parameter(self.building_scenarios['linearization_zone_air_temperature_cool']))
    ...
    def define_heat_transfer_hvac_tu(self):
        for index, row in self.building_zones.iterrows():
            if row['hvac_tu_type'] != '':
                self.control_matrix.at[index + '_temperature', index + '_tu_heat_air_volume_flow'] = (
                        self.control_matrix.at[index + '_temperature', index + '_tu_heat_air_volume_flow']
                        + self.parse_parameter('heat_capacity_air')
                ...
                self.control_matrix.at[index + '_temperature', index + '_tu_cool_air_volume_flow'] = (
                        self.control_matrix.at[index + '_temperature', index + '_tu_cool_air_volume_flow']
                        + self.parse_parameter('heat_capacity_air')
                        * (self.parse_parameter(row['tu_supply_air_temperature_setpoint'])
                           - self.parse_parameter(self.building_scenarios['linearization_zone_air_temperature_cool']))
        ...
        for index_exterior, row_exterior in self.building_surfaces_exterior.iterrows():
            ...
            if self.parse_parameter(row_exterior['heat_capacity']) != 0:
                self.disturbance_matrix.at[
                    index_exterior + '_temperature', 'irradiation_' + row_exterior['direction_name']
                ] = (
                        self.disturbance_matrix.at[
                            index_exterior + '_temperature', 'irradiation_' + row_exterior['direction_name']
                        ]
                ...
                self.disturbance_matrix.at[
                    row_exterior['zone_name'] + '_temperature', 'irradiation_' + row_exterior['direction_name']] = (
                        self.disturbance_matrix.at[
                            row_exterior['zone_name'] + '_temperature', 'irradiation_' + row_exterior['direction_name']]
                        + 1 / (
                                1
                                + (2 * self.parse_parameter(row_exterior['thermal_resistance_surface'])
                                   + 1 / self.parse_parameter('heat_transfer_coefficient_surface_to_zone'))
                                * (self.parse_parameter('heat_transfer_coefficient_surface_to_ambient')
                                   + self.parse_parameter(row_exterior['heat_transfer_coefficient_surface_to_ground'])
                                   + self.parse_parameter(row_exterior['heat_transfer_coefficient_surface_to_sky']))
            ...
            for index_interior, row_interior in pd.concat([
                ...
                self.building_surfaces_interior[:][
                    self.building_surfaces_interior['zone_adjacent_name'] == row_exterior['zone_name']],
                self.building_surfaces_adiabatic[:][
                    self.building_surfaces_adiabatic['zone_name'] == row_exterior['zone_name']]]).iterrows():
                if self.parse_parameter(row_interior['heat_capacity']) != 0:
                    # Received by the wall
                    self.disturbance_matrix.at[
                        index_interior + '_temperature', 'irradiation_' + row_exterior['direction_name']
                    ] = (
                            self.disturbance_matrix.at[
                                index_interior + '_temperature', 'irradiation_' + row_exterior['direction_name']
                            ]
                    ...
                    self.disturbance_matrix.at[
                        row_interior['zone_name'] + '_temperature', 'irradiation_' + row_exterior['direction_name']] = (
                            self.disturbance_matrix.at[
                                row_interior['zone_name'] + '_temperature', 'irradiation_' + row_exterior[
                                    'direction_name']]
                            + (1
                               - 1 / (
                                       1
                    ...
                    self.disturbance_matrix.at[
                        row_interior['zone_name'] + '_temperature', 'irradiation_' + row_exterior['direction_name']
                            ...
                            + (1
                               - 1 / (
                                       2 * (1 + self.parse_parameter(row_interior['thermal_resistance_surface'])
                                            * self.parse_parameter('heat_transfer_coefficient_surface_to_zone')))
                               )
                            * self.parse_parameter(row_exterior['irradiation_transfer_coefficient'])
                            * self.parse_parameter(row_exterior['surface_area'])
            ...
            for index_interior, row_interior in pd.concat([
                ...
                self.building_surfaces_interior[:][
                    self.building_surfaces_interior['zone_name'] == row_exterior['zone_name']],
                self.building_surfaces_interior[:][
                    self.building_surfaces_interior['zone_adjacent_name'] == row_exterior['zone_name']]]).iterrows():
                if self.parse_parameter(row_interior['heat_capacity']) == 0:
    ...
    def define_heat_transfer_emission_sky(self):
        for index_exterior, row_exterior in self.building_surfaces_exterior.iterrows():
            ...
            if self.parse_parameter(row_exterior['heat_capacity']) != 0:
                self.state_matrix.at[index_exterior + '_temperature', index_exterior + '_temperature'] = (
                        self.state_matrix.at[index_exterior + '_temperature', index_exterior + '_temperature']
                        - 1 / (
                                1
                                + self.parse_parameter(row_exterior['thermal_resistance_surface'])
                ...
                self.disturbance_matrix.at[index_exterior + '_temperature', 'sky_temperature'] = (
                        self.disturbance_matrix.at[index_exterior + '_temperature', 'sky_temperature']
                        + 1 / (
                                1
                                + self.parse_parameter(row_exterior['thermal_resistance_surface'])
            ...
            # Heat flow received by zone through wall
                ...
                self.state_matrix.at[
                    row_exterior['zone_name'] + '_temperature', row_exterior['zone_name'] + '_temperature'] = (
                        self.state_matrix.at[
                            row_exterior['zone_name'] + '_temperature', row_exterior['zone_name'] + '_temperature'
                        ]
                        - 1 / (
                ...
                self.disturbance_matrix.at[row_exterior['zone_name'] + '_temperature', 'sky_temperature'] = (
                        self.disturbance_matrix.at[row_exterior['zone_name'] + '_temperature', 'sky_temperature']
                        + 1 / (
                                1
                                + (2 * self.parse_parameter(row_exterior['thermal_resistance_surface'])
            ...
            if self.parse_parameter(row_exterior['window_wall_ratio']) != 0:
                self.state_matrix.at[
                    row_exterior['zone_name'] + '_temperature', row_exterior['zone_name'] + '_temperature'] = (
                        ...
                        - 1 / (
                                1
                                + (2 * self.parse_parameter(row_exterior['thermal_resistance_window'])
                                   + 1 / self.parse_parameter('heat_transfer_coefficient_surface_to_zone'))
                                * (self.parse_parameter('heat_transfer_coefficient_surface_to_ambient')
                                   + self.parse_parameter(row_exterior['heat_transfer_coefficient_surface_to_ground'])
                                   + self.parse_parameter(row_exterior['heat_transfer_coefficient_surface_to_sky']))
    ...
    def define_heat_transfer_emission_environment(self):
        for index_exterior, row_exterior in self.building_surfaces_exterior.iterrows():
            ...
            if self.parse_parameter(row_exterior['heat_capacity']) != 0:
                self.state_matrix.at[
                    index_exterior + '_temperature', index_exterior + '_temperature'] = (
                        self.state_matrix.at[index_exterior + '_temperature', index_exterior + '_temperature']
                        - 1 / (
                                1
                                + self.parse_parameter(row_exterior['thermal_resistance_surface'])
                ...
                self.disturbance_matrix.at[
                    index_exterior + '_temperature', 'ambient_air_temperature'] = (
                        self.disturbance_matrix.at[index_exterior + '_temperature', 'ambient_air_temperature']
                        + 1 / (
                                1
                                + self.parse_parameter(row_exterior['thermal_resistance_surface'])
                                * (self.parse_parameter('heat_transfer_coefficient_surface_to_ambient')
            ...
            # Heat flow received by zone through wall
                ...
                self.disturbance_matrix.at[
                    row_exterior['zone_name'] + '_temperature', 'ambient_air_temperature'] = (
                        self.disturbance_matrix.at[
                            row_exterior['zone_name'] + '_temperature', 'ambient_air_temperature'
                        ]
                        + 1 / (
        ...
        if self.building_scenarios['co2_model_type'][0] != '':
            for index, row in self.building_zones.iterrows():
                if (row['hvac_ahu_type'] != '') | (row['window_type'] != ''):
                    self.state_matrix.at[index + '_co2_concentration', index + '_co2_concentration'] = (
                            self.state_matrix.at[index + '_co2_concentration', index + '_co2_concentration']
                            - (
                                    self.parse_parameter(self.building_scenarios['linearization_ventilation_rate_per_square_meter'])
                                    / self.parse_parameter(row['zone_height'])
                            )
                    )
                    if row['hvac_ahu_type'] != '':
                        self.control_matrix.at[index + '_co2_concentration', index + '_ahu_heat_air_volume_flow'] = (
                                self.control_matrix.at[index + '_co2_concentration', index + '_ahu_heat_air_volume_flow']
                                - (
                        ...
                        self.control_matrix.at[index + '_co2_concentration', index + '_ahu_cool_air_volume_flow'] = (
                                self.control_matrix.at[index + '_co2_concentration', index + '_ahu_cool_air_volume_flow']
                                - (
                                        self.parse_parameter(self.building_scenarios['linearization_co2_concentration']
                                        )
                    ...
                    if row['window_type'] != '':
                        self.control_matrix.at[index + '_co2_concentration', index + '_window_air_flow'] = (
                                self.control_matrix.at[index + '_co2_concentration', index + '_window_air_flow']
                                - (
                                        self.parse_parameter(self.building_scenarios['linearization_co2_concentration']
                                        )
                    ...
                    # Ventilation through infiltration is not considered there, because:
                    # - During the day, exfiltrations only? The building is ventilated so it may be over pressurised
                    # - During the night, the stack effect + wind should happen, but should be really low with such building
                    self.disturbance_matrix.at[
                        index + '_co2_concentration', row['internal_gain_type'] + '_occupancy'] = (
                            self.disturbance_matrix.at[
                                index + '_co2_concentration', row['internal_gain_type'] + '_occupancy']
                            + (
    ...
    def define_humidity_transfer_hvac_ahu(self):
        if self.building_scenarios['humidity_model_type'][0] != '':
            for index, row in self.building_zones.iterrows():
                if row['hvac_ahu_type'] != '':
                    self.state_matrix.at[index + '_absolute_humidity', index + '_absolute_humidity'] = (
                            self.state_matrix.at[index + '_absolute_humidity', index + '_absolute_humidity']
                            - (
                                    self.parse_parameter(
                                        self.building_scenarios['linearization_ventilation_rate_per_square_meter'])
                    ...
                    self.control_matrix.at[index + '_absolute_humidity', index + '_ahu_heat_air_volume_flow'] = (
                            self.control_matrix.at[index + '_absolute_humidity', index + '_ahu_heat_air_volume_flow']
                            - (
                                (self.parse_parameter(self.building_scenarios['linearization_zone_air_humidity_ratio'])
                                 - humid_air_properties(
                                    'W',
                                    'T', self.parse_parameter(row['ahu_supply_air_temperature_setpoint']) + 273.15,
                                    'R', self.parse_parameter(row['ahu_supply_air_relative_humidity_setpoint']),
                                    'P', 101325)
                                 )
                                / self.parse_parameter(row['zone_height'])
                                / self.parse_parameter(row['zone_area'])
                    ...
                    self.control_matrix.at[index + '_absolute_humidity', index + '_ahu_cool_air_volume_flow'] = (
                            self.control_matrix.at[index + '_absolute_humidity', index + '_ahu_cool_air_volume_flow']
                            - (
                                    (self.parse_parameter(self.building_scenarios['linearization_zone_air_humidity_ratio'])
                                     - humid_air_properties(
                    ...
                    if row['window_type'] != '':
                        self.control_matrix.at[index + '_absolute_humidity', index + '_window_air_flow'] = (
                                self.control_matrix.at[index + '_absolute_humidity', index + '_window_air_flow']
                                - (
                                        (self.parse_parameter(
                    ...
                    # Ventilation through infiltration is not considered there, because:
                    # - During the day, exfiltrations only? The building is ventilated so it may be over pressurised
                    # - During the night, the stack effect + wind should happen, but should be really low with such kind of building
                    self.disturbance_matrix.at[
                        index + '_absolute_humidity', row['internal_gain_type'] + '_occupancy'] = (
                            self.disturbance_matrix.at[
                                index + '_absolute_humidity', row['internal_gain_type'] + '_occupancy']
                            + (
    ...
    def define_output_zone_temperature(self):
        for index, row in self.building_zones.iterrows():
            self.state_output_matrix.at[index + '_temperature', index + '_temperature'] = 1

    def define_output_zone_co2_concentration(self):
        if self.building_scenarios['co2_model_type'][0] != '':
            for index, row in self.building_zones.iterrows():
                if (row['hvac_ahu_type'] != '') | (row['window_type'] != ''):
                    self.state_output_matrix.at[index + '_co2_concentration', index + '_co2_concentration'] = 1

    def define_output_zone_humidity(self):
        if self.building_scenarios['humidity_model_type'][0] != '':
            for index, row in self.building_zones.iterrows():
                if (row['hvac_ahu_type'] != ''):
                    self.state_output_matrix.at[index + '_absolute_humidity', index + '_absolute_humidity'] = 1

    def define_output_hvac_generic_power(self):
        for index, row in self.building_zones.iterrows():
            if row['hvac_generic_type'] != '':
                self.control_output_matrix.at[
                    index + '_generic_heat_power', index + '_generic_heat_thermal_power'
                ] = (
                        self.control_output_matrix.at[
                            index + '_generic_heat_power', index + '_generic_heat_thermal_power'
                        ]
                        + 1 / self.parse_parameter(row['generic_heating_efficiency'])
                )
                self.control_output_matrix.at[
                    index + '_generic_cool_power', index + '_generic_cool_thermal_power'
                ] = (
                        self.control_output_matrix.at[
                            index + '_generic_cool_power', index + '_generic_cool_thermal_power'
                        ]
                        + 1 / self.parse_parameter(row['generic_cooling_efficiency'])
                )
            ...
            if row['hvac_ahu_type'] != '':
                ...
                if (
                    ...
                    if self.parse_parameter(
                            ...
                            humid_air_properties(
                                'W',
                                'R', self.parse_parameter(row['ahu_supply_air_relative_humidity_setpoint']) / 100,
                                'T', self.parse_parameter(
                                    self.building_scenarios['linearization_ambient_air_temperature']) + 273.15,
                                'P', 101325):
                        ...
                        delta_enthalpy_cooling_recovery = min(
                            ...
                            self.parse_parameter(row['ahu_return_air_heat_recovery_efficiency']) * (
                                humid_air_properties(
                                    'H',
                                    'T', self.parse_parameter(
                                        self.building_scenarios['linearization_zone_air_temperature_heat']) + 273.15,
                                    'W', self.parse_parameter(
                                        self.building_scenarios['linearization_ambient_air_humidity_ratio']),
                                    'P', 101325)
                                - humid_air_properties(
                        ...
                        delta_enthalpy_ahu_cooling = (
                                humid_air_properties(
                                    'H',
                                    'R', 1,
                                    'W', humid_air_properties(
                                        'W',
                                        'T', self.parse_parameter(row['ahu_supply_air_temperature_setpoint']) + 273.15,
                                        'R', self.parse_parameter(
                                            row['ahu_supply_air_relative_humidity_setpoint']) / 100,
                                        'P', 101325),
                                    'P', 101325)
                                - humid_air_properties(
                                    'H',
                ...
                # Matrix entries
                self.control_output_matrix.at[
                    index + '_ahu_heat_electric_power', index + '_ahu_heat_air_volume_flow'
                ] = (
                        self.control_output_matrix.at[
                            index + '_ahu_heat_electric_power', index + '_ahu_heat_air_volume_flow'
                        ]
                        + self.parse_parameter('density_air')
                        * ((abs(delta_enthalpy_ahu_cooling) - abs(delta_enthalpy_cooling_recovery))
                           / self.parse_parameter(row['ahu_cooling_efficiency'])
                           + (abs(delta_enthalpy_ahu_heating) - abs(delta_enthalpy_heating_recovery))
                           / self.parse_parameter(row['ahu_heating_efficiency'])
                           + self.parse_parameter(row['ahu_fan_efficiency']))
                )
                self.control_output_matrix.at[
                    index + '_ahu_cool_electric_power', index + '_ahu_cool_air_volume_flow'
                ] = (
                        self.control_output_matrix.at[
                            index + '_ahu_cool_electric_power', index + '_ahu_cool_air_volume_flow'
                        ]
    ...
    def define_output_hvac_tu_electric_power(self):
        for index, row in self.building_zones.iterrows():
            if row['hvac_tu_type'] != '':
                ...
                # Matrix entries
                self.control_output_matrix.at[
                    index + '_tu_heat_electric_power', index + '_tu_heat_air_volume_flow'
                ] = (
                        self.control_output_matrix.at[
                            index + '_tu_heat_electric_power', index + '_tu_heat_air_volume_flow'
                        ]
                ...
                self.control_output_matrix.at[
                    index + '_tu_cool_electric_power', index + '_tu_cool_air_volume_flow'
                ] = (
                        self.control_output_matrix.at[
                            index + '_tu_cool_electric_power', index + '_tu_cool_air_volume_flow']
                        + self.parse_parameter('density_air')
    ...
    def define_output_fresh_air_flow(self):
        for index, row in self.building_zones.iterrows():
            if row['hvac_ahu_type'] != '':
                self.control_output_matrix.at[
                    index + '_total_fresh_air_flow', index + '_ahu_heat_air_volume_flow'
                ] = 1
                self.control_output_matrix.at[
                    index + '_total_fresh_air_flow', index + '_ahu_cool_air_volume_flow'
                ] = 1
            if row['window_type'] != '':
                self.control_output_matrix.at[
                    index + '_total_fresh_air_flow', index + '_window_air_flow'
                ] = 1
            '''
            if (row['window_type'] != '') | (row['hvac_ahu_type'] != ''):
                self.disturbance_output_matrix.at[index + '_total_fresh_air_flow', 'constant'] = (
                        self.disturbance_output_matrix.at[index + '_total_fresh_air_flow', 'constant']
                        + self.parse_parameter(row['infiltration_rate'])
                        * self.parse_parameter(row['zone_area'])
                        * self.parse_parameter(row['zone_height'])
    ...
    def define_output_ahu_fresh_air_flow(self):
        for index, row in self.building_zones.iterrows():
            if row['hvac_ahu_type'] != '':
                self.control_output_matrix.at[
                    index + '_ahu_fresh_air_flow', index + '_ahu_heat_air_volume_flow'
                ] = 1
                self.control_output_matrix.at[
                    index + '_ahu_fresh_air_flow', index + '_ahu_cool_air_volume_flow'
                ] = 1
    ...
    def define_output_window_fresh_air_flow(self):
        for index, row in self.building_zones.iterrows():
            if (row['window_type'] != ''):
                self.control_output_matrix.at[
                    index + '_window_fresh_air_flow', index + '_window_air_flow'
                ] = 1
    ...
    def load_disturbance_timeseries(self, conn):
        ...
        # Define constant (only when it should be considered)
        if self.define_constant:
            building_constant = pd.Series(
                np.ones(self.time_vector.shape[0]),
                self.time_vector,
                name='constant'
            )
    ...
    def define_output_constraint_timeseries(self, conn):
        ...
        self.output_constraint_timeseries_maximum = pd.DataFrame(
            np.ones((self.time_vector.shape[0], self.n_outputs)) * np.infty,
            self.time_vector,
            self.i_outputs.index
        )
        ...
        for index_zone, row_zone in self.building_zones.iterrows():
            ...
            for row_time in self.time_vector:
                ...
                self.output_constraint_timeseries_minimum.at[
                    row_time, index_zone + '_temperature'
                ] = self.parse_parameter(building_zone_constraint_profile['minimum_air_temperature'][
                                             int(constraint_profile_index_time(
                                                 row_time.to_datetime64().astype('int64')))
                                         ])
                self.output_constraint_timeseries_maximum.at[
                    row_time, index_zone + '_temperature'
                ] = self.parse_parameter(building_zone_constraint_profile['maximum_air_temperature'][
                                             int(constraint_profile_index_time(
                                                 row_time.to_datetime64().astype('int64')))
                                         ])
                if (row_zone['hvac_ahu_type'] != '') | (row_zone['window_type'] != ''):
                    if self.building_scenarios['demand_controlled_ventilation'][0] != '':
                        if self.building_scenarios['co2_model_type'][0] != '':
                            self.output_constraint_timeseries_maximum.at[
                                row_time, index_zone + '_co2_concentration'
                            ] = self.parse_parameter(
                        ...
                        if row_zone['hvac_ahu_type'] != '':
                            ...
                            self.output_constraint_timeseries_minimum.at[
                                row_time, index_zone + '_window_fresh_air_flow'
                            ] = self.parse_parameter(
                                building_zone_constraint_profile['minimum_fresh_air_flow_per_area_no_dcv'][
                                    int(constraint_profile_index_time(row_time.to_datetime64().astype('int64')))
                                ]
                ...
                if self.building_scenarios['humidity_model_type'][0] != '':
                    if row_zone['hvac_ahu_type'] != '':
                        self.output_constraint_timeseries_minimum.at[
                            row_time, index_zone + '_absolute_humidity'
                        ] = humid_air_properties(
                                    'W',
                                    'R', self.parse_parameter(
                                    ...
                                    'T', self.parse_parameter(
                                        self.building_scenarios['linearization_zone_air_temperature_cool']) + 273.15,
                                    'P', 101325)
                        self.output_constraint_timeseries_maximum.at[
                            row_time, index_zone + '_absolute_humidity'
                        ] = humid_air_properties(
