...
def translate_J_plus_to_DH_requirements(DAYS_IN_YEAR, HOURS_IN_DAY, thermal_data, hex_design_data, hex_type, temperature_data, U, cp):
    ...
    return mdot_DH_result, T_return_DH_result, T_supply_DH_result, mdot_heating_result,  mdot_dhw_result, T_r1_dhw_result
...
for i in range(len(building_list_array)):
    ...
    results = pd.DataFrame({"mdot_DH_result":mdot_DH_result_flat,"T_return_DH_result":T_return_DH_result_flat,"T_supply_DH_result":T_supply_DH_result_flat, "mdot_heating_result":mdot_heating_result_flat,"mdot_dhw_result":mdot_dhw_result_flat, "T_r1_dhw_result":T_r1_dhw_result_flat})
    fName_result = building_list_array[i,0] + "_result2.csv"
