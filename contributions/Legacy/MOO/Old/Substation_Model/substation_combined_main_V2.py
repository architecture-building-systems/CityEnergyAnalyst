...
data_path = "/Users/Tim/Desktop/ETH/Masterarbeit/Tools/Python_Testcases/ZONE_3_B/ZONE_3"
results_path = "/Users/Tim/Desktop/ETH/Masterarbeit/Tools/Python_Testcases/ZONE_3_B/Results"
...
import numpy as np
import pandas as pd
import substation_combined_functions_V2 as fn
...
T_supply_max_all_buildings = np.array(fn.find_max_temp_of_buildings(building_list, "tshs", DAYS_IN_YEAR, HOURS_IN_DAY))
# ... gives maximum return temperature to building = T_r2 after Palsson et al
T_return_max_all_buildings = fn.find_max_temp_of_buildings(building_list, "trhs", DAYS_IN_YEAR, HOURS_IN_DAY)

print "start reading data from:", data_path 
...
for i in range(len(building_list_array)):
    ...
    hex_design_data = fn.design_inhouse_exchangers(Q_heating_design, Q_dhw_design, T_return_min_global, T_supply_max_global, mdot_internal_dhw_design, mdot_internal_heating_design, Q_cool_max, T_cool_return_max_global, T_cool_supply_min_global, mdot_internal_cool_design, U_cool, hex_type, U, cp)
                           
    # calculate the demands
    result_arrays = fn.translate_J_plus_to_DH_requirements(DAYS_IN_YEAR, HOURS_IN_DAY, thermal_data, hex_design_data, hex_type, temperature_data, U, U_cool, cp, mdot_step_counter, mdot_cool_step_counter, T_cool_return_max_global, T_cool_supply_min_global)
    ...
    results = pd.DataFrame({"mdot_DH_result":mdot_DH_result_flat,"T_return_DH_result":T_return_DH_result_flat,"T_supply_DH_result":T_supply_DH_result_flat, "mdot_heating_result":mdot_heating_result_flat,"mdot_dhw_result":mdot_dhw_result_flat, "mdot_DC_result":mdot_cool_result_flat, "T_r1_dhw_result":T_r1_dhw_result_flat,"T_r1_heating_result":T_r1_heating_result_flat, "T_return_DC_result":T_r1_cool_result_flat, "A_hex_heating_design":A_hex_heating_design, "A_hex_dhw_design":A_hex_dhw_design, "A_hex_cool_design":A_hex_cool_design, "Q_heating":Q_heating_array_flat, "Q_dhw":Q_dhw_array_flat, "Q_cool":Q_cool_array_flat})
