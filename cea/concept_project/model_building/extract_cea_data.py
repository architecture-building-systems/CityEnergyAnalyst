...
def extract_cea_inputs_files(scenario_data_path, scenario):
    # Get dbf
    zone_occupancy_dbf = Dbf5(os.path.join(scenario_data_path, scenario, 'inputs', 'building-properties', 'occupancy.dbf'))
    zone_dbf = Dbf5(os.path.join(scenario_data_path, scenario, 'inputs', 'building-geometry', 'zone.dbf'))
    architecture_dbf = Dbf5(os.path.join(scenario_data_path, scenario, 'inputs', 'building-properties', 'architecture.dbf'))
    technical_systems_dbf = Dbf5(os.path.join(scenario_data_path, scenario, 'inputs', 'building-properties', 'technical_systems.dbf'))
    supply_systems_dbf = Dbf5(os.path.join(scenario_data_path, scenario, 'inputs', 'building-properties', 'supply_systems.dbf'))
...
def process_occupancy_schedules(
    ...
    for i in range(occupancy_types_cardinal):
        ...
        if len(index_occupancy) != i + 1:
            print(
                    'Error: no Probability of occupancy (daily) found for occupancy type '
                    + str(i)
                    + ' in occupancy_schedules.xlsx. Please check the spelling.')
            quit()
        if len(index_lighting_appliances) != i + 1:
            print(
                    'Error: no Probability of use of lighting and appliances (daily) found for occupancy type '
                    + str(i)
                    + ' in occupancy_schedules.xlsx. Please check the spelling.')
            quit()
        if len(index_monthly_use) != i + 1:
            print(
                    'Error: no Probability of use (monthly) found for occupancy type '
                    + str(i)
                    + ' in occupancy_schedules.xlsx. Please check the spelling.')
            quit()
        if len(index_occupancy_density) != i + 1:
            print(
                    'Error: no Occupancy density found for occupancy type '
                    + str(i)
                    + ' in occupancy_schedules.xlsx. Please check the spelling.'
            )
            quit()
