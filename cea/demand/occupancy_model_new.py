...
def calc_schedules(region, list_uses, archetype_schedules, occupancy, archetype_values, conditioned_area, occupancy_model):
    """
    ...
    # # calculate occupant schedules
    # if occupancy_model == 'deterministic':
    #     occupant_schedules = calc_occupant_schedules(archetype_values['people'], list_uses, occupancy, archetype_schedules[0], conditioned_area)
    # else:
    #     occupant_schedules =
...
def calc_occupant_schedules(occupant_densities, list_uses, occupancy, archetype_schedules, conditioned_area):
    current_schedule = np.zeros(8760)
    normalizing_value = 0.0
    occupant_schedules = {}
    for num in range(len(list_uses)):
        if occupant_densities[num] != 0: # do not consider when the value is 0
            current_share_of_use = occupancy[list_uses[num]]
            share_time_occupancy_density = occupant_densities[num] * current_share_of_use
            normalizing_value += share_time_occupancy_density
            current_schedule = np.vectorize(calc_average)(current_schedule, archetype_schedules[num],
                                                          share_time_occupancy_density)
        occupant_schedules[list_uses[num]] = current_schedule

    return occupant_schedules
