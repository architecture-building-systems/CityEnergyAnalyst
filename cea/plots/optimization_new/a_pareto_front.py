...
def plot_pareto_front(objectives, objective_values_dict):
    ...
    # Create a scatter plot for each pair of objectives
...
def main(config=cea.config.Configuration()):
    ...
    for run_id in run_ids:
        ...
        individual_supply_system_results = [
            locator.get_centralized_optimization_optimal_supply_systems_summary_file(run_id, subfolder)
        ...
        if 'Cost_USD' in objectives:
            individual_network_results = [
                locator.get_centralized_optimization_detailed_network_performance_file(subfolder)
                for subfolder in os.listdir(optimisation_results) if not subfolder == 'debugging'
            ]
